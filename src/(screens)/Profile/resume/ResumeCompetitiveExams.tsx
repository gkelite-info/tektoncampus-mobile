...
export default function ResumeCompetitiveExams() {
  ...
  if (isPageLoading) {
    return <View className="flex-1 bg-white rounded-xl  items-center justify-center p-4">
                ...
                <Text className="text-gray-400 mt-2" style={{ fontFamily: fonts.regular }}>{t("Auto.Common.Loadingexams", "Loading exams...")}</Text>
            </View>;
  }
  return <ScrollView className="flex-1 bg-[#f6f7f9] p-4">
            <View className="bg-white rounded-lg p-6  mb-10">
                <View className="flex-row justify-between items-center mb-6">
                    <Text className="text-lg text-[#000000]" style={{ fontFamily: fonts.semiBold }}>{t("Auto.Common.CompetitiveExam", "Competitive Exams")}</Text>
                </View>
        ...
        return <View key={index} className="border border-gray-200 rounded-lg p-3 mb-2 bg-gray-50 flex-row justify-between items-center">
                        <View>
                            <Text className="text-gray-800" style={{ fontFamily: fonts.medium }}>{item.examName}</Text>
                            <Text className="text-sm text-gray-500" style={{ fontFamily: fonts.regular }}>{t("Auto.Common.Score", "Score:")}{item.score}</Text>
                        </View>
                ...
                {exams.length === 0 && <Text className="text-gray-500 mb-4" style={{ fontFamily: fonts.regular }}>{t("Auto.Common.Noexamsaddedyet", "No exams added yet.")}</Text>}

                <View className="border border-gray-200 rounded-xl p-4 bg-white mt-4 ">
                    <Text className="text-sm text-[#43C17A] mb-3" style={{ fontFamily: fonts.medium }}>{t("Auto.Common.AddNewExam", "Add New Exam")}</Text>
                    
                    <Text className="text-sm text-[#282828] mb-1" style={{ fontFamily: fonts.medium }}>{t("Auto.Common.ExamName", "Exam Name *")}</Text>
                    ...
                    <Text className="text-sm text-[#282828] mb-1" style={{ fontFamily: fonts.medium }}>{t("Auto.Common.Score", "Score *")}</Text>
                    ...
                    <TouchableOpacity onPress={handleAdd} disabled={isSaving} className="bg-[#43C17A] px-4 py-2.5 rounded-lg items-center">
                        <Text className="text-white" style={{ fontFamily: fonts.bold }}>{isSaving ? t("Dashboard.profile.Saving", "Saving...") : t("Dashboard.profile.AddExam", "Add Exam")}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </ScrollView>;
