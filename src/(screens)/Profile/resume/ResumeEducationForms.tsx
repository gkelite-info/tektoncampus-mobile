...
interface SharedProps {
    ...
    studentId: number; collegeId: number;
}

export function ResumePrimaryEducationForm({ data, onSave, studentId }: SharedProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await onSave({
                resumeEducationDetailId: data?.resumeEducationDetailId,
                studentId,
                schoolName,
                board,
                mediumOfStudy,
...
export function ResumeSecondaryEducationForm({ data, onSave, onDelete, studentId }: SharedProps) {
...
export function ResumeUndergraduateEducationForm({ data, onSave, onDelete, studentId }: SharedProps) {
...
export function ResumePhdEducationForm({ data, onSave, onDelete, studentId }: SharedProps) {
...
export function ResumeMastersEducationForm({ data, onSave, onDelete, studentId, collegeId }: SharedProps) {
    const [institutionName, setInstitutionName] = useState(data?.institutionName || "");
    const [courseName, setCourseName] = useState(data?.courseName || "");
    const [specialization, setSpecialization] = useState(data?.specialization || "");
    const [cgpa, setCgpa] = useState(data?.cgpa?.toString() || "");
    const [percentage, setPercentage] = useState(data?.percentage?.toString() || "");
    const [startYear, setStartYear] = useState(data?.startYear?.toString() || "");
    const [yearOfPassing, setYearOfPassing] = useState(data?.yearOfPassing?.toString() || "");
    const [isLoading, setIsLoading] = useState(false);

    const handleSubmit = async () => {
        if (!institutionName || !courseName || !startYear) {
    ...
    return (
        <View className="border border-gray-200 rounded-lg p-4 bg-white mb-4">
            <View className="flex-row justify-between items-center mb-4">
                <Text className="text-[#43C17A] font-bold text-lg">Master's Education</Text>
                {onDelete && (
                    <TouchableOpacity onPress={onDelete} className="p-2">
                        <Trash size={20} color="#ef4444" />
                    </TouchableOpacity>
            ...
            <View className="flex-row gap-3 mb-3">
                <View className="flex-1">
                    <Text className="text-sm font-medium text-[#282828] mb-1">Course Name*</Text>
                    <TextInput className="border border-[#CCCCCC] rounded-md px-3 py-2" value={courseName} onChangeText={setCourseName} placeholder="e.g. M.Tech" />
                </View>
                <View className="flex-1">
                    <Text className="text-sm font-medium text-[#282828] mb-1">Specialization</Text>
                    <TextInput className="border border-[#CCCCCC] rounded-md px-3 py-2" value={specialization} onChangeText={setSpecialization} placeholder="e.g. Computer Science" />
                </View>
            </View>
            ...
            <View className="flex-row gap-3 mb-3">
                <View className="flex-1">
                    <Text className="text-sm font-medium text-[#282828] mb-1">CGPA</Text>
                    <TextInput className="border border-[#CCCCCC] rounded-md px-3 py-2" value={cgpa} onChangeText={setCgpa} placeholder="e.g. 8.5" keyboardType="numeric" />
                </View>
                <View className="flex-1">
                    <Text className="text-sm font-medium text-[#282828] mb-1">Percentage</Text>
                    <TextInput className="border border-[#CCCCCC] rounded-md px-3 py-2" value={percentage} onChangeText={setPercentage} placeholder="e.g. 85" keyboardType="numeric" />
                </View>
            </View>
            ...
            <View className="flex-row gap-3 mb-4">
                ...
                <View className="flex-1">
                    <Text className="text-sm font-medium text-[#282828] mb-1">Year of Passing</Text>
                    <TextInput className="border border-[#CCCCCC] rounded-md px-3 py-2" value={yearOfPassing} onChangeText={setYearOfPassing} placeholder="YYYY" keyboardType="numeric" maxLength={4} />
                </View>
            </View>

            <TouchableOpacity onPress={handleSubmit} disabled={isLoading} className="bg-[#43C17A] py-2 rounded-md items-center">
                <Text className="text-white font-bold">{isLoading ? "Saving..." : "Save Master's Education"}</Text>
            </TouchableOpacity>
        </View>
    );
}
