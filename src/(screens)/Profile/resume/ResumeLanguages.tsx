...
export default function ResumeLanguages() {const { t } = useTranslation();
  ...
  <View key={lang} className="bg-white border border-gray-200 rounded-full pl-4 pr-2 py-1.5 m-1 flex-row items-center gap-2">
            <Text className="text-gray-700 text-sm" style={{ fontFamily: fonts.medium }}>{lang}</Text>
  ...
  if (loading) {
    return (
      <View className="flex-1 bg-white rounded-xl  items-center justify-center p-4">
                ...
                <Text className="text-gray-400 mt-2" style={{ fontFamily: fonts.regular }}>{t("Auto.Common.Loadinglanguage", "Loading languages...")}</Text>
            </View>);
  ...
  return (
    <ScrollView className="flex-1 bg-white rounded-xl " contentContainerStyle={{ padding: 16 }}>
            <View className="flex-row justify-between items-center mb-6">
                <Text className="text-lg text-[#000000]" style={{ fontFamily: fonts.semiBold }}>{t("Auto.Common.Languages", "Languages")}</Text>
            ...
            <View className="mb-10">
                <Text className="text-sm text-[#282828] mb-2" style={{ fontFamily: fonts.medium }}>{t("Auto.Common.SelectedLanguag", "Selected Languages")}</Text>
                <View className="border border-[#C0C0C0] rounded-md p-3 min-h-[68px] flex-row flex-wrap items-center">
                    {selected.length === 0 ?
          <Text className="text-sm text-gray-400" style={{ fontFamily: fonts.italic }}>{t("Auto.Common.Nolanguagessele", "No languages selected.")}</Text> :
          ...
          selected.map(renderPill)
                ...
                <TouchableOpacity onPress={() => setIsModalOpen(true)} className="border-2 border-dashed border-[#43C17A] rounded-lg p-4 mt-6 items-center justify-center bg-[#43C17A]/5">
                    <Text className="text-[#43C17A]" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.AddLanguage", "+ Add Language")}</Text>
                </TouchableOpacity>
