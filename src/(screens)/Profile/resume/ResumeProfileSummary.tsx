  ...
  if (isPageLoading) {
    return (
      <View className="flex-1 bg-white rounded-xl  items-center justify-center p-4">
                ...
                <Text className="text-gray-400 mt-2">{t("Auto.Common.LoadingATSEngin", "Loading ATS Engine...")}</Text>
            </View>);
    ...
    <ScrollView className="flex-1 bg-[#f6f7f9] p-4">
      ...
      <View className="bg-white rounded-lg p-6  mb-4 border border-gray-100 flex-row justify-between items-center">
                    <View className="flex-1">
                        <Text className="text-sm font-semibold text-gray-500 uppercase tracking-wide">{t("Auto.Common.ResumeATSScore", "Resume ATS Score")}</Text>
                        <Text className="text-3xl font-bold mt-1" style={{ color: atsResult.color }}>{atsResult.total}/100</Text>
                        <Text className="text-sm font-medium mt-1" style={{ color: atsResult.color }}>{atsResult.label}{t("Auto.Common.Match", "Match")}</Text>
                    </View>
                    <View className="flex-1 ml-4">
                        <Text className="text-xs text-gray-500 mb-1">{t("Auto.Common.TopImprovementT", "Top Improvement Tips:")}</Text>
          ...
          <>
                            ...
                            <Text className="text-[#43C17A] font-bold ml-2">{t("Auto.Common.GeneratingwithA", "Generating with AI...")}</Text>
                        </> :

          <Text className="text-[#43C17A] font-bold">{t("Auto.Common.GenerateSummary", "\u2728 Generate Summary with AI")}</Text>
          }
                </TouchableOpacity>

                {showOptions && generatedOptions.length > 0 &&
        <View className="mt-4">
                        <Text className="text-sm font-semibold text-[#43C17A] mb-2">{t("Auto.Common.AISuggestionsTa", "AI Suggestions (Tap to apply)")}</Text>
          ...
          <TouchableOpacity
            ...
            <View className="bg-white rounded-lg p-6  mb-10 border border-purple-100">
                <View className="flex-row items-center mb-2">
                    <Text className="text-lg font-semibold text-purple-700">{t("Auto.Common.ATSJDGapAnalyze", "\uD83C\uDFAF ATS JD Gap Analyzer")}</Text>
                </View>
                <Text className="text-sm text-gray-500 mb-4">{t("Auto.Common.PasteaJobDescri", "Paste a Job Description to see missing skills and tailor your profile summary to match.")}</Text>
          ...
          <>
                            ...
                            <Text className="text-white font-bold ml-2">{t("Auto.Common.AnalyzingJD", "Analyzing JD...")}</Text>
                        </> :

          <Text className="text-white font-bold">{t("Auto.Common.ScanResumeagain", "Scan Resume against JD")}</Text>
          }
                </TouchableOpacity>

                {gapAnalysis &&
        <View className="mt-6">
                        <Text className="text-sm font-semibold text-green-600 mb-2">{t("Auto.Common.MatchingSkills", "\u2705 Matching Skills")}</Text>
            ...
            <View key={i} className="bg-green-100 border border-green-300 rounded-full px-3 py-1">
                            ...
                            {gapAnalysis.matching.length === 0 && <Text className="text-xs text-gray-400">{t("Auto.Common.Nomatchingskill", "No matching skills found.")}</Text>}
                        </View>

                        <Text className="text-sm font-semibold text-red-500 mb-2">{t("Auto.Common.MissingSkillsAd", "\u274C Missing Skills (Add these!)")}</Text>
            ...
            <View key={i} className="bg-red-100 border border-red-300 rounded-full px-3 py-1">
                            ...
                            {gapAnalysis.missing.length === 0 && <Text className="text-xs text-gray-400">{t("Auto.Common.Nomissingskills", "No missing skills detected.")}</Text>}
                        </View>
                    </View>
        }
            </View>
