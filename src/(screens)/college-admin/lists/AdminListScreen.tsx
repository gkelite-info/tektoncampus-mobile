...
export default function AdminListScreen({ navigation }: any) {
    ...
    const renderItem = ({ item }: { item: AdminListRow }) => {
        // Map AdminListRow to AdminDetail expected by AdminProfileCard
        const mappedData = {
            ...
            mobile: item.mobile,
            gender: item.gender,
            dateOfJoining: "—", // Not present in row
            collegeEducationId: 0,
            eduType: item.educationType,
            branchCount: item.branches ? item.branches.split(",").length : 0,
            isActive: true
        };
