...
import { getFacultyListData, FacultyRow, FacultyListData } from "@/lib/helpers/collegeAdmin/getFacultyListData";
import AdminProfileCard from "../dashboard/components/AdminProfileCard";

const ROWS_PER_PAGE = 10;

export default function FacultyListScreen({ navigation }: any) {
    ...
    const load = useCallback(async (page: number, searchTerm: string) => {
        ...
        setIsFetching(true);
        try {
            const result = await getFacultyListData(
                collegeId,
                page,
                ROWS_PER_PAGE,
                { search: searchTerm || undefined }
            );
            setRows(result.faculty);
    ...
    const renderItem = ({ item }: { item: FacultyRow }) => {
        const mappedData = {
            adminId: Number(item.facultyId) || 0,
            ...
            branchCount: item.branchCode ? 1 : 0, // Using branchCount field for branch
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                data={rows}
                keyExtractor={(item, index) => String(item.facultyId) + index}
