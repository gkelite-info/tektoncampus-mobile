...
import { getFinanceListData, FinanceRow } from "@/lib/helpers/collegeAdmin/getFinanceListData";
import AdminProfileCard from "../dashboard/components/AdminProfileCard";

const ROWS_PER_PAGE = 10;

export default function FinanceListScreen({ navigation }: any) {
    ...
    const load = useCallback(async (page: number, searchTerm: string) => {
        ...
        setIsFetching(true);
        try {
            const result = await getFinanceListData(
                collegeId,
                page,
                ROWS_PER_PAGE,
                { search: searchTerm || undefined }
            );
            setRows(result.finance);
    ...
    const renderItem = ({ item }: { item: FinanceRow }) => {
        const mappedData = {
            adminId: Number(item.financeManagerId) || 0,
        ...
        <View style={styles.container}>
            ...
            <View style={styles.searchContainer}>
                <TextInput
                    ...
                    placeholder="Search by Finance Manager Name or ID"
                    value={search}
            ...
            <FlatList
                data={rows}
                keyExtractor={(item, index) => String(item.financeManagerId) + index}
                renderItem={renderItem}
                contentContainerStyle={styles.listContainer}
                ListEmptyComponent={<Text style={styles.emptyText}>No finance managers found.</Text>}
            />
