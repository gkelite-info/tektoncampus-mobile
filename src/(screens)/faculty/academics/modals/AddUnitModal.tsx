...
export default function AddUnitModal({ visible, onClose, onSave, subjectDetails }: AddUnitModalProps) {const { t } = useTranslation();
      ...
      <View className="flex-1 bg-black/40 pt-10">
        <View className="flex-1 bg-white rounded-t-3xl">
          <View className="px-6 py-5 border-b border-gray-100 flex-row items-center justify-between">
            <Text className="text-xl font-bold text-[#282828]">{t("Auto.Common.AddUnit", "Add Unit")}</Text>
            <TouchableOpacity onPress={onClose}>
              <Text className="text-[#43C17A] font-bold text-sm">{t("Auto.Common.Cancel", "Cancel")}</Text>
            </TouchableOpacity>
          </View>

          <ScrollView className="flex-1 px-6 pt-5" contentContainerStyle={{ paddingBottom: 100 }}>
            <Text className="text-[#525252] text-xs mb-6">{t("Auto.Common.Trackprogressad", "Track progress, add lessons, and manage course content across all your batches.")}
            ...
            <View className="flex-row flex-wrap gap-x-4 gap-y-4 mb-6">
              <View className="w-[47%]">
                <Text className="text-sm font-semibold text-[#282828] mb-1.5">{t("Auto.Common.Education", "Education")}</Text>
              ...
              <View className="w-[47%]">
                <Text className="text-sm font-semibold text-[#282828] mb-1.5">{t("Auto.Common.Branch", "Branch")}</Text>
              ...
              <View className="w-[47%]">
                <Text className="text-sm font-semibold text-[#282828] mb-1.5">{t("Auto.Common.Year", "Year")}</Text>
              ...
              <View className="w-[47%]">
                  <Text className="text-sm font-semibold text-[#282828] mb-1.5">{t("Auto.Common.Semester", "Semester")}</Text>
              ...
              <View className="w-full mt-2">
                <Text className="text-sm font-semibold text-[#282828] mb-1.5">{t("Auto.Common.SubjectName", "Subject Name")}</Text>
            ...
            <View className="mb-6 z-50">
              <Text className="text-sm font-semibold text-[#282828] mb-1.5">{t("Auto.Common.UnitName", "Unit Name *")}</Text>
              <TextInput
              ...
              <View className="mt-3 border border-[#BBF7D0] bg-[#F0FDF4] rounded-lg p-4">
                  <View className="flex-row items-center justify-between mb-3">
                    <Text className="text-xs font-semibold text-[#43C17A]">{t("Auto.Common.AISuggestedTopi", "AI Suggested Topics")}</Text>
                  ...
                  <View className="flex-row items-center gap-3">
                      ...
                      <Square size={16} color="#43C17A" />
                          ...
                          <Text className="text-[10px] font-bold text-[#43C17A]">{t("Auto.Common.SelectAll", "Select All")}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={handleAddSelectedStaged} className="bg-[#43C17A] px-2 py-1 rounded">
                          <Text className="text-white text-[10px] font-bold">{t("Auto.Common.AddSelected", "Add Selected")}</Text>
                        </TouchableOpacity>
                      </View>
                  }
                  </View>
                ...
                <View className="flex-row items-center gap-2">
                      ...
                      <Text className="text-xs text-[#43C17A]">{t("Auto.Common.Generatingtopic", "Generating topics...")}</Text>
                    </View> :
              ...
              <View className="flex-row items-center gap-2 mt-4">
                ...
                <TouchableOpacity onPress={handleAddTopic} className="bg-[#43C17A] px-4 py-2.5 rounded-lg">
                  <Text className="text-white text-xs font-bold">{t("Auto.Common.Add", "Add")}</Text>
                </TouchableOpacity>
              </View>
            </View>

            <View className="mb-6">
              <Text className="text-sm font-semibold text-[#282828] mb-1.5">{t("Auto.Common.UnitNumber", "Unit Number *")}</Text>
              <TextInput
                className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 bg-white"
          ...
          <View className="p-4 border-t border-gray-200 bg-white">
            <TouchableOpacity
              ...
              <Text className="text-white font-bold text-base">{t("Auto.Common.SaveUnit", "Save Unit")}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
