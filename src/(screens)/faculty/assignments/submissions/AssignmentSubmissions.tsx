...
import { View, Text, TouchableOpacity, FlatList, ActivityIndicator, TextInput, Linking } from 'react-native';
    ...
    return (
      <View className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-3">
        ...
        <View className="flex-row items-center justify-between mt-1 mb-3">
          <Text className="text-xs text-gray-500">Submitted: {item.date}</Text>
          {item.filePath && (
        ...
        {isEditing ? (
          <View className="bg-gray-50 p-3 rounded-lg border border-gray-200 gap-3">
            <View className="flex-row items-center gap-3">
              <Text className="text-sm font-bold text-gray-700 w-16">Marks:</Text>
              <TextInput
            ...
            <View className="flex-row items-center gap-3">
              <Text className="text-sm font-bold text-gray-700 w-16">Feedback:</Text>
              <TextInput
            ...
            <View className="flex-row items-center gap-3">
              <Text className="text-sm font-bold text-gray-700 w-16">Status:</Text>
              <View className="flex-1 flex-row gap-2">
                ...
                >
                  <Text className={`text-xs font-bold ${tempData?.status === 'Evaluated' ? 'text-[#43C17A]' : 'text-gray-500'}`}>Evaluated</Text>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                >
                  <Text className={`text-xs font-bold ${tempData?.status === 'Pending' ? 'text-[#FFBB70]' : 'text-gray-500'}`}>Pending</Text>
                </TouchableOpacity>
              </View>
            </View>
          ...
          <View className="flex-row items-center justify-between border-t border-gray-100 pt-3">
            <View className="flex-1">
              <Text className="text-xs font-bold text-gray-800">Marks: <Text className="font-medium text-gray-600">{item.marks || '-'}</Text></Text>
              <Text className="text-xs font-bold text-gray-800 mt-1" numberOfLines={1}>Notes: <Text className="font-medium text-gray-600 italic">{item.feedback || '-'}</Text></Text>
            </View>
  ...
  return (
    <View className="flex-1 bg-[#F4F4F4]">
      <View className="bg-white px-4 py-4 border-b border-gray-200 flex-row items-center gap-3 shadow-sm z-10 pt-12">
        ...
        <Text className="text-xl font-bold text-[#16284F]">Assignment Submissions</Text>
      </View>
      ...
      <View className="p-4 flex-1">
        ...
        {loading ? (
          ...
          <View className="items-center justify-center mt-10">
            <Text className="text-gray-500 font-semibold">No submissions found.</Text>
          </View>
        ) : (
          <FlatList
            data={filtered}
