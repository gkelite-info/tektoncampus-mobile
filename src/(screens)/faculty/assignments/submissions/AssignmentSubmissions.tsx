    ...
    return (
      <View className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-3">
        <View className="flex-row justify-between items-start mb-2">
          <View>
            <Text className="text-base text-gray-800" style={{ fontFamily: fonts.bold }}>{item.name}</Text>
            <Text className="text-sm text-green-600" style={{ fontFamily: fonts.semiBold }}>{item.roll}</Text>
          </View>
          ...
          item.status === 'Pending' ? 'bg-[#FFF1E2]' : 'bg-gray-100'}`
            ...
            } style={{ fontFamily: fonts.bold }}>{item.status}</Text>
          </View>
        </View>

        <View className="flex-row items-center justify-between mt-1 mb-3">
          <Text className="text-xs text-gray-500" style={{ fontFamily: fonts.regular }}>{t("Auto.Common.Submitted", "Submitted:")} {item.date}</Text>
          {item.filePath &&
          <TouchableOpacity onPress={() => handleViewFile(item.filePath!)} className="flex-row items-center gap-1 bg-red-50 px-2 py-1 rounded-md">
              ...
              <Text className="text-xs text-red-500 max-w-[100px]" numberOfLines={1} style={{ fontFamily: fonts.medium }}>{item.file}</Text>
            </TouchableOpacity>
          }
        </View>
        ...
        <View className="bg-gray-50 p-3 rounded-lg border border-gray-200 gap-3">
            <View className="flex-row items-center gap-3">
              <Text className="text-sm text-gray-700 w-16" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Marks", "Marks:")}</Text>
              <TextInput
            ...
            <View className="flex-row items-center gap-3">
              <Text className="text-sm text-gray-700 w-16" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Feedback", "Feedback:")}</Text>
              <TextInput
            ...
            <View className="flex-row items-center gap-3">
              <Text className="text-sm text-gray-700 w-16" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Status", "Status:")}</Text>
              <View className="flex-1 flex-row gap-2">
                ...
                className={`px-2 py-1 rounded border ${tempData?.status === 'Evaluated' ? 'border-[#43C17A] bg-[#E7F7EE]' : 'border-gray-300 bg-white'}`}>
                  ...
                  <Text className={`text-xs ${tempData?.status ==='Evaluated' ? 'text-[#43C17A]' : 'text-gray-500'}`} style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Evaluated", "Evaluated")}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                className={`px-2 py-1 rounded border ${tempData?.status === 'Pending' ? 'border-[#FFBB70] bg-[#FFF1E2]' : 'border-gray-300 bg-white'}`}>
                  ...
                  <Text className={`text-xs ${tempData?.status ==='Pending' ? 'text-[#FFBB70]' : 'text-gray-500'}`} style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Pending", "Pending")}</Text>
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View className="flex-row items-center justify-end gap-3 mt-2">
              <TouchableOpacity onPress={() => setEditingId(null)} className="px-3 py-1.5 rounded bg-gray-200">
                <Text className="text-sm text-gray-600" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Cancel", "Cancel")}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={confirmSave} className="px-3 py-1.5 rounded bg-[#43C17A]">
                <Text className="text-sm text-white" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Save", "Save")}</Text>
              </TouchableOpacity>
            </View>
          </View> :
        ...
        <View className="flex-row items-center justify-between border-t border-gray-100 pt-3">
            <View className="flex-1">
              <Text className="text-xs text-gray-800" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Marks", "Marks:")} <Text className="text-gray-600" style={{ fontFamily: fonts.medium }}>{item.marks || '-'}</Text></Text>
              <Text className="text-xs text-gray-800 mt-1" numberOfLines={1} style={{ fontFamily: fonts.bold }}>{t("Auto.Common.Notes", "Notes:")} <Text className="text-gray-600" style={{ fontFamily: fonts.italic }}>{item.feedback || '-'}</Text></Text>
            </View>
  ...
  return (
    <View className="flex-1 bg-[#F4F4F4]">
      <View className="bg-white px-4 py-4 border-b border-gray-200 flex-row items-center gap-3 shadow-sm z-10 pt-12">
        ...
        <Text className="text-xl text-[#16284F]" style={{ fontFamily: fonts.bold }}>{t("Auto.Common.AssignmentSubmi", "Assignment Submissions")}</Text>
      </View>
      ...
      <View className="p-4 flex-1">
        ...
        <View className="mb-4">
            <View className="flex-row gap-3">
              <View className="flex-1 bg-[#E2DAFF] rounded-xl p-3 items-center justify-center border border-[#D5CAFF] shadow-sm">
                ...
                <Text className="text-sm text-[#282828]" style={{ fontFamily: fonts.bold }}>{formatDateInt(assignmentDetails.dueDate)}</Text>
              </View>
              <View className="flex-1 bg-[#FFEDDA] rounded-xl p-3 items-center justify-center border border-[#FFDFBC] shadow-sm">
                ...
                <Text className="text-sm text-[#282828]" style={{ fontFamily: fonts.bold }}>{assignmentDetails.totalMarks}</Text>
              </View>
              <View className="flex-1 bg-[#E6FBEA] rounded-xl p-3 items-center justify-center border border-[#BDECC9] shadow-sm">
                ...
                <Text className="text-sm text-[#282828]" style={{ fontFamily: fonts.bold }}>{assignmentDetails.totalSubmissions}</Text>
              </View>
            </View>
          </View>
        }
        ...
        <View className="flex-row gap-2 mb-4">
          ...
          <TouchableOpacity
            ...
            className={`px-3 py-1.5 rounded-full ${filter === f ? 'bg-[#43C17A]' : 'bg-white border border-gray-200'}`}>
              ...
              <Text className={`text-xs ${filter === f ?'text-white' : 'text-gray-500'}`} style={{ fontFamily: fonts.bold }}>{f}</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <View className="items-center justify-center mt-10">
            <Text className="text-gray-500" style={{ fontFamily: fonts.semiBold }}>{t("Auto.Common.Nosubmissionsfo", "No submissions found.")}</Text>
          </View> :
