...
function AttendanceToggle({ value, onChange, disabled }: { value: string, onChange: (val: string) => void, disabled: boolean }) {
  if (disabled) {
    let bg = "bg-gray-100", text = "text-gray-600";
    if (value === "Present") { bg = "bg-[#43C17A1C]"; text = "text-[#43C17A]"; }
    else if (value === "Absent") { bg = "bg-red-100"; text = "text-red-600"; }
    else if (value === "Leave") { bg = "bg-blue-100"; text = "text-blue-600"; }
    
    return (
      <View className={`px-3 py-1.5 rounded-lg items-center justify-center ${bg}`}>
  ...
  return (
    <View className="flex-row items-center gap-1.5">
      <TouchableOpacity 
        ...
        className={`w-8 h-8 rounded-full items-center justify-center border ${value === 'Present' ? 'bg-[#43C17A] border-[#43C17A]' : 'bg-white border-gray-300'}`}
      >
        <Text className={`text-xs font-bold ${value === 'Present' ? 'text-white' : 'text-gray-500'}`}>P</Text>
      </TouchableOpacity>
      
      <TouchableOpacity 
        onPress={() => onChange("Absent")}
        className={`w-8 h-8 rounded-full items-center justify-center border ${value === 'Absent' ? 'bg-red-500 border-red-500' : 'bg-white border-gray-300'}`}
      >
        <Text className={`text-xs font-bold ${value === 'Absent' ? 'text-white' : 'text-gray-500'}`}>A</Text>
      </TouchableOpacity>

      <TouchableOpacity 
        onPress={() => onChange("Leave")}
        className={`w-8 h-8 rounded-full items-center justify-center border ${value === 'Leave' ? 'bg-blue-500 border-blue-500' : 'bg-white border-gray-300'}`}
      >
        <Text className={`text-xs font-bold ${value === 'Leave' ? 'text-white' : 'text-gray-500'}`}>L</Text>
      </TouchableOpacity>
    </View>
  );
}
...
export default function StuAttendanceTable({
  ...
  const updateReason = (id: string, value: string) => {
    if (!isEditing) return;
    setStudents(students.map(s => s.id === id ? { ...s, reason: value } : s));
  };
  ...
  const bulkUpdate = (status: UIStudent["attendance"]) => {
    if (!isEditing) return;
    setStudents(students.map(s => selectedIds.includes(s.id) ? { ...s, attendance: status } : s));
    setSelectedIds([]);
  };
    ...
    <View className="flex-1 w-full flex-col space-y-4">
      <View className="flex-col gap-4 mb-2">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-3 py-1">
          {!isTopicMode && onFilterChange && (
            <TouchableOpacity 
              ...
              className="flex-row items-center bg-[#43C17A1C] px-4 py-2 rounded-full mr-2"
            >
          ...
          {!isTopicMode && onFilterChange && (
            <TouchableOpacity 
              onPress={() => setShowSectionFilter(true)}
              className="flex-row items-center bg-[#43C17A1C] px-4 py-2 rounded-full mr-2"
            >
              <Text className="text-[#43C17A] text-sm font-medium mr-2">
          ...
          <TouchableOpacity 
            ...
            className="flex-row items-center bg-[#43C17A1C] px-4 py-2 rounded-full mr-2"
          >
            <Text className="text-[#43C17A] text-sm font-medium mr-2">Sort: {sort}</Text>
            <CaretDown size={14} color="#43C17A" weight="bold" />
          </TouchableOpacity>
        </ScrollView>
        ...
        <View className="flex-row justify-between items-center mb-2">
            ...
            {isEditing && (
              <>
                <TouchableOpacity
                  ...
                  className={`w-5 h-5 rounded items-center justify-center border ${selectedIds.length === filtered.length && filtered.length > 0 ? 'bg-[#43C17A] border-[#43C17A]' : 'border-gray-300 bg-white'}`}
                >
                  {selectedIds.length === filtered.length && filtered.length > 0 && <CheckCircle size={16} color="white" weight="fill" />}
                </TouchableOpacity>
                <Text className="ml-2 text-sm text-gray-600 font-medium">Select All</Text>
              </>
            )}
          </View>
          ...
          {!isEditing ? (
            <TouchableOpacity
              onPress={onEditClick}
              disabled={(!isTopicMode && !selectedClass) || students.length === 0 || loadingFilters}
              className={`flex-row items-center justify-center gap-2 bg-[#43C17A] px-4 py-2 rounded-lg ${((!isTopicMode && !selectedClass) || students.length === 0 || loadingFilters) ? 'opacity-50' : ''}`}
            >
              <PencilSimple size={16} color="white" weight="bold" />
              <Text className="text-white text-sm font-medium">Edit Attendance</Text>
            </TouchableOpacity>
          ) : (
            <View className="flex-row items-center gap-2">
              {onCancelEditClick && (
                <TouchableOpacity onPress={onCancelEditClick} className="bg-gray-200 px-3 py-2 rounded-lg">
                  <Text className="text-gray-700 text-sm font-medium">Cancel Edit</Text>
                </TouchableOpacity>
              )}
              {onMarkCancelClick && !isCancellingMode && (
                <TouchableOpacity onPress={onMarkCancelClick} className="flex-row items-center justify-center bg-[#FFBB70] px-3 py-2 rounded-lg">
                  <Prohibit size={16} color="white" weight="bold" />
                  <Text className="text-white text-sm font-medium ml-1">Class Cancel</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
                onPress={handleSaveAttendance}
                disabled={saving || loadingFilters}
                className={`bg-[#43C17A] px-4 py-2 rounded-lg ${saving ? 'opacity-50' : ''}`}
              >
      ...
      {isEditing && selectedIds.length > 0 && !loadingFilters && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row bg-white border border-gray-100 rounded-xl px-4 py-2 shadow-sm mb-4">
          ...
          <TouchableOpacity onPress={() => bulkUpdate("Present")} className="flex-row items-center gap-1 px-3 py-1 bg-[#43C17A] rounded-lg mr-2">
            ...
            <Text className="text-white text-xs font-medium">Present</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity onPress={() => bulkUpdate("Leave")} className="flex-row items-center gap-1 px-3 py-1 bg-blue-500 rounded-lg mr-2">
            ...
            <Text className="text-white text-xs font-medium">Leave</Text>
          </TouchableOpacity>
        </ScrollView>
      )}
      ...
      <View className="flex-1 w-full pb-10">
        {filtered.length > 0 ? filtered.map((s, index) => (
          <View key={s.id} className={`w-full bg-white rounded-2xl shadow-sm border border-gray-100 mb-3 overflow-hidden ${selectedIds.includes(s.id) ? "border-[#43C17A] border-2" : ""}`}>
            <View className="p-4 flex-row items-center justify-between">
              ...
              <View className="flex-row items-center flex-1">
                {isEditing && (
                  <TouchableOpacity
                    disabled={!isEditing}
                    onPress={() => toggleSelectOne(s.id)}
                    className={`w-5 h-5 rounded border items-center justify-center mr-3 ${selectedIds.includes(s.id) ? 'bg-[#43C17A] border-[#43C17A]' : 'border-gray-300'}`}
                  >
                ...
                {s.photo ? (
                  <Image source={{ uri: s.photo }} className="w-10 h-10 rounded-full bg-gray-200" />
                ) : (
                ...
                <View className="ml-3 flex-1 pr-2">
                  ...
                  <Text className="text-gray-500 text-[11px] font-medium mt-0.5">
                    ID: {s.roll} • Attendance: {s.percentage}
                  </Text>
                </View>
              </View>
              ...
              <View className="flex-row items-center">
                {s.attendance === "Class Cancel" ? (
                  <View className="rounded-lg bg-gray-100 py-1.5 px-3 items-center">
                    <Text className="text-gray-600 text-xs font-bold">Cancelled</Text>
                  </View>
                ) : (
                  <AttendanceToggle value={s.attendance} onChange={(val) => updateAttendance(s.id, val as any)} disabled={!isEditing} />
                )}
              </View>
              ...
              isEditing ? (
                <View className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex-row items-center">
                  <View className="bg-white rounded-lg px-3 py-2 border border-gray-200 flex-1 flex-row items-center">
                    <TextInput
                      value={s.reason || ""}
                      onChangeText={(val) => updateReason(s.id, val)}
                ...
                <View className="px-4 py-2.5 bg-gray-50 border-t border-gray-100 flex-row items-center">
                  <Text className="text-xs text-gray-500 italic flex-1" numberOfLines={3}>
                    Reason: {s.reason}
                  </Text>
                </View>
              ) : null
          ...
          <View className="bg-white rounded-t-3xl p-6 min-h-[300px]">
            ...
            <ScrollView>
              {classes.map(c => (
                <TouchableOpacity 
                  key={c.id} 
                  onPress={() => { onFilterChange && onFilterChange("class", c.id); setShowClassFilter(false); }}
      ...
      <Modal visible={showSectionFilter} transparent animationType="slide">
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-white rounded-t-3xl p-6 min-h-[300px]">
            <Text className="text-lg font-bold mb-4">Select Section</Text>
            <ScrollView>
              {sections.map(s => (
                <TouchableOpacity 
                  key={s.id} 
                  onPress={() => { onFilterChange && onFilterChange("section", s.id); setShowSectionFilter(false); }}
                  className="py-3 border-b border-gray-100"
                >
                  <Text className={`text-base ${selectedSection === s.id ? 'text-[#43C17A] font-bold' : 'text-gray-700'}`}>Section {s.name}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
      ...
      <Modal visible={showSortFilter} transparent animationType="slide">
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-white rounded-t-3xl p-6 min-h-[300px]">
            ...
            <ScrollView>
              {["All", "Present", "Absent", "Leave", "Class Cancel"].map(s => (
                <TouchableOpacity 
                  key={s} 
