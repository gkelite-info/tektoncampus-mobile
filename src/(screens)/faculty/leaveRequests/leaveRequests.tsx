...
import { View, Text, FlatList, TouchableOpacity, TextInput, ActivityIndicator, StyleSheet, Platform, Linking } from 'react-native';
...
import {
  ...
  fetchTaggedLeaves,
  fetchTaggedLeaveCounts,
} from '@/lib/helpers/faculty/leaveRequests/facultyLeaveAPI';
...
export default function LeaveRequestsScreen() {
  ...
  useEffect(() => {
    if (!userId) return;
    getFacultyIdByUserId(userId as number)
      .then((id) => setFacultyId(id))
      .catch(() => Toast.show({ type: 'error', text1: 'Faculty context not found' }));
  }, [userId]);
    ...
    try {
      if (mainTab === 'students') {
        const [tableRes, countRes] = await Promise.all([
          fetchStudentLeavesForFaculty(facultyId, page, 10, activeTab, debouncedSearch),
        ...
        const [tableRes, countRes] = await Promise.all([
          fetchTaggedLeaves(facultyId, page, 10, activeTab, debouncedSearch),
          fetchTaggedLeaveCounts(facultyId)
        ]);
        setTableData(page === 1 ? tableRes.data : [...tableData, ...tableRes.data]);
        setTotalItems(tableRes.totalCount);
        setCounts(countRes);
        ...
        const [tableRes, countRes] = await Promise.all([
          fetchFacultyLeaves(facultyId, page, 10, activeTab, debouncedSearch),
  ...
  const renderStatsCard = (id: string, label: string, value: number, icon: any, activeBg: string, faintBg: string) => {
    ...
    return (
      <TouchableOpacity 
        onPress={() => { setActiveTab(id as any); setPage(1); }}
  ...
  const renderStudentLeaveCard = ({ item }: { item: any }) => (
    <View className="bg-white rounded-xl p-4 mb-3 border border-gray-100 flex-col">
      <View className="flex-row justify-between items-start mb-3">
        <View className="flex-row gap-3 flex-1">
           <Avatar src={item.photo} size={40} />
           <View className="flex-1">
              <Text className="text-[#282828] font-bold text-sm">{item.name}</Text>
              <Text className="text-gray-500 text-xs">ID: <Text className="font-semibold text-gray-700">{item.rollNo}</Text> • {item.branch}</Text>
      ...
      <View className="bg-gray-50 p-3 rounded-lg flex-row justify-between mb-3 border border-gray-100">
        <View>
          <Text className="text-xs text-gray-500 mb-0.5">Date Range</Text>
          <Text className="text-xs text-[#282828] font-semibold">{item.fromDate} - {item.toDate}</Text>
        </View>
        <View className="items-end">
          <Text className="text-xs text-gray-500 mb-0.5">Days</Text>
          <Text className="text-xs text-[#282828] font-semibold">{item.days} Day(s)</Text>
        </View>
      </View>
      ...
      {item.attachments && item.attachments.length > 0 && (
        <View className="flex-row gap-2 mb-3">
          {item.attachments.map((url: string, idx: number) => (
             <TouchableOpacity key={idx} onPress={() => Linking.openURL(url)} className="flex-row items-center bg-blue-50 px-2 py-1 rounded-md border border-blue-100">
                <Paperclip size={12} color="#2563EB" />
                <Text className="text-[10px] text-blue-600 font-medium ml-1">Attach {idx + 1}</Text>
             </TouchableOpacity>
          ))}
        </View>
      )}
      ...
      <View className="flex-row items-center justify-between border-t border-gray-100 pt-3">
         <TouchableOpacity 
           onPress={() => { setSelectedLeaveData(item); setIsDetailsModalOpen(true); }}
           className="px-2"
         >
         ...
         {item.status === 'pending' ? (
           <View className="flex-row gap-2">
             <TouchableOpacity onPress={() => setConfirmModal({ isOpen: true, leaveId: item.id, action: "Rejected" })} className="bg-[#FFE5E5] px-3 py-1.5 rounded-full">
               <Text className="text-[#FF4B4B] text-[10px] font-bold">Reject</Text>
             </TouchableOpacity>
             <TouchableOpacity onPress={() => setConfirmModal({ isOpen: true, leaveId: item.id, action: "Approved" })} className="bg-[#E7F8EE] px-3 py-1.5 rounded-full">
               <Text className="text-[#43C17A] text-[10px] font-bold">Approve</Text>
             </TouchableOpacity>
           </View>
         ) : (
           <TouchableOpacity onPress={() => setConfirmModal({ isOpen: true, leaveId: item.id, action: item.status === 'approved' ? "Rejected" : "Approved" })} className="flex-row items-center bg-gray-100 px-2 py-1.5 rounded-full border border-gray-200">
             <PencilSimple size={12} color="#6B7280" />
             <Text className="text-gray-600 text-[10px] font-bold ml-1">Change Status</Text>
           </TouchableOpacity>
         )}
      </View>
    </View>
  ...
  const renderMyLeaveCard = ({ item }: { item: any }) => (
    <View className="bg-white rounded-xl p-4 mb-3 border border-gray-100 flex-col">
       ...
       <View className="flex-row justify-between my-2">
        <View>
          <Text className="text-[10px] text-gray-400 font-medium">From</Text>
          <Text className="text-xs text-gray-700 font-bold">{item.fromDate}</Text>
        </View>
        <View className="items-end">
          <Text className="text-[10px] text-gray-400 font-medium">To</Text>
       ...
       <View className="bg-gray-50 p-2.5 rounded-lg border border-gray-100 mb-2">
         <Text className="text-[10px] text-gray-400 font-medium mb-1">Reason</Text>
         <Text className="text-xs text-[#282828] font-medium" numberOfLines={2}>{item.description}</Text>
       </View>
       <View className="flex-row justify-between items-center mt-1">
         <Text className="text-gray-400 text-[10px]">Total Days:</Text>
         <Text className="text-[#282828] text-xs font-bold">{item.days}</Text>
       </View>
    </View>
  );

  const renderTaggedLeaveCard = ({ item }: { item: any }) => (
    <View className="bg-white rounded-xl p-4 mb-3 border border-gray-100 flex-col">
       <View className="flex-row justify-between items-start mb-3">
         ...
         <View className={`px-2 py-1 rounded border ${
            item.status === 'approved' ? 'bg-[#E7F8EE] border-[#43C17A]' : 
            item.status === 'rejected' ? 'bg-[#FFE5E5] border-[#FF4B4B]' : 
            'bg-[#FFF4EB] border-[#FFB874]'
         }`}>
           <Text className={`text-[10px] font-bold ${
              item.status === 'approved' ? 'text-[#43C17A]' : 
              item.status === 'rejected' ? 'text-[#FF4B4B]' : 
              'text-[#FFB874]'
       ...
       <View className="bg-[#F5F7FA] rounded-lg p-3 mb-3">
         <View className="flex-row justify-between mb-2">
            <Text className="text-[11px] text-gray-500 font-medium">Duration</Text>
            <Text className="text-[11px] font-bold text-[#282828]">{item.fromDate} - {item.toDate}</Text>
         </View>
         <View className="flex-row justify-between">
            <Text className="text-[11px] text-gray-500 font-medium">Total Days</Text>
            <Text className="text-[11px] font-bold text-[#43C17A]">{item.days} Days</Text>
         </View>
       </View>
  ...
  return (
    <View className="flex-1 bg-[#F5F7FA]" style={{ paddingTop: insets.top + 120 }}>
      <View className="px-4 pt-2 pb-2">
        ...
        <View className="flex-row bg-gray-100 rounded-full p-1 mb-4 relative">
          <View style={[StyleSheet.absoluteFill, { padding: 4, zIndex: 0 }]}>
            <MotiView
              style={{ 
                width: '33.33%',
                height: '100%',
          ...
          >
            <Text className={`text-[10px] sm:text-xs font-bold ${mainTab === 'students' ? 'text-white' : 'text-gray-500'}`}>Students</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            onPress={() => setMainTab('tagged')}
            className="flex-1 py-2 rounded-full items-center z-10"
          >
            <Text className={`text-[10px] sm:text-xs font-bold ${mainTab === 'tagged' ? 'text-white' : 'text-gray-500'}`}>Tagged</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            onPress={() => setMainTab('my_leaves')}
            className="flex-1 py-2 rounded-full items-center z-10"
          >
            <Text className={`text-[10px] sm:text-xs font-bold ${mainTab === 'my_leaves' ? 'text-white' : 'text-gray-500'}`}>My Leaves</Text>
          </TouchableOpacity>
        </View>
        ...
        {mainTab === 'my_leaves' && (
           <TouchableOpacity 
             ...
             className="bg-[#16284F] w-full py-3 rounded-lg items-center justify-center mb-4"
           >
              <Text className="text-white text-sm font-bold">Request Leave</Text>
           </TouchableOpacity>
        )}
        ...
        <View className="flex-row items-center bg-white rounded-full px-4 h-[42px] border border-gray-200">
          ...
          <TextInput
            className="flex-1 ml-2 text-sm text-[#282828]"
      ...
      {isLoading && page === 1 ? (
        ...
        <FlatList
          ...
          renderItem={mainTab === 'students' ? renderStudentLeaveCard : (mainTab === 'my_leaves' ? renderMyLeaveCard : renderTaggedLeaveCard)}
          ...
          ListEmptyComponent={
            <View className="items-center justify-center pt-10">
              ...
              <Text className="text-gray-500 mt-4 font-medium text-sm">No leave requests found.</Text>
            </View>
          }
        />
      )}
      ...
      {isRequestModalOpen && (
        <RequestLeaveModal 
          ...
          facultyId={facultyId!} 
          onSuccess={() => { setIsRequestModalOpen(false); handleRefresh(); }} 
        />
      )}
      ...
      {confirmModal.isOpen && (
        <ConfirmStatusModal 
          ...
          leaveId={confirmModal.leaveId!} 
          onClose={() => setConfirmModal({ isOpen: false, leaveId: null, action: null })} 
          onSuccess={() => { setConfirmModal({ isOpen: false, leaveId: null, action: null }); loadData(false); }} 
        />
      )}

      {isDetailsModalOpen && selectedLeaveData && (
