      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1 justify-center items-center bg-black/40 px-4">
        ...
        <View className="bg-white w-full rounded-2xl shadow-xl overflow-hidden max-w-sm">
          <View className="flex-row justify-between items-center p-5 border-b border-gray-100">
            <Text className="text-lg font-bold text-[#282828]">{t("Auto.Common.RequestLeave", "Request Leave")}</Text>
          ...
          gap: 16
            ...
            <View className="gap-1.5 z-50">
              <Text className="text-sm font-semibold text-[#282828]">{t("Auto.Common.LeaveType", "Leave Type *")}</Text>
              ...
              {showTypeDropdown && <View className="absolute top-[65px] left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-sm z-50 overflow-hidden">
                ...
                return <TouchableOpacity key={idx} onPress={() => {
                  ...
                  setShowTypeDropdown(false);
                      ...
                      <Text className="text-sm text-[#282828]">{type}{t("Auto.Common.Leave", "Leave")}</Text>
                    </TouchableOpacity>;
              })}
                </View>}
            </View>
            ...
            <View className="flex-row gap-4">
              <View className="flex-1 gap-1.5">
                <Text className="text-sm font-semibold text-[#282828]">{t("Auto.Common.StartDate", "Start Date *")}</Text>
                <TextInput value={startDate} onChangeText={setStartDate} placeholder={t("Auto.Attr.YYYYMMDD", "YYYY-MM-DD")} placeholderTextColor="#9CA3AF" className="border border-gray-200 rounded-lg px-3 py-3 text-sm text-[#282828]" />
                
              </View>
              <View className="flex-1 gap-1.5">
                <Text className="text-sm font-semibold text-[#282828]">{t("Auto.Common.EndDate", "End Date *")}</Text>
                <TextInput value={endDate} onChangeText={setEndDate} placeholder={t("Auto.Attr.YYYYMMDD", "YYYY-MM-DD")} placeholderTextColor="#9CA3AF" className="border border-gray-200 rounded-lg px-3 py-3 text-sm text-[#282828]" />
                
              </View>
            </View>
            {}
            <View className="gap-1.5 z-10">
              <Text className="text-sm font-semibold text-[#282828]">{t("Auto.Common.Description", "Description *")}</Text>
            ...
            <View className="gap-1.5 z-0">
              <Text className="text-sm font-semibold text-[#282828]">{t("Auto.Common.TagFacultyOptio", "Tag Faculty (Optional)")}</Text>
              ...
              {showTagDropdown && <View className="border border-gray-200 rounded-lg shadow-sm mt-1 max-h-40 overflow-hidden bg-gray-50">
                  ...
                  return <TouchableOpacity key={fac.id} onPress={() => toggleTag(fac.id)} className={`flex-row items-center px-3 py-2 border-b border-gray-100 ${isSelected ? 'bg-[#E7F8EE]' : 'bg-white'}`}>
                    ...
                    {faculties.length === 0 && <View className="px-3 py-4 items-center">
                        <Text className="text-xs text-gray-400">{t("Auto.Common.Nofacultiesavai", "No faculties available to tag")}</Text>
                      </View>}
                  </ScrollView>
                </View>}
            </View>
          ...
          <View className="p-4 border-t border-gray-100 flex-row gap-3">
            <TouchableOpacity onPress={onClose} disabled={isSubmitting} className="flex-1 py-3 bg-gray-100 rounded-lg items-center justify-center">
              ...
              <Text className="text-[#525252] font-bold text-sm">{t("Auto.Common.Cancel", "Cancel")}</Text>
            </TouchableOpacity>
