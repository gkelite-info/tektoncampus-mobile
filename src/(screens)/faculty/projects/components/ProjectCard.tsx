      ...
      return <View key={index} style={tw`bg-white rounded-2xl shadow-sm border border-gray-100 px-4 py-4`}>
                    ...
                    <View style={tw`flex-row justify-between items-start gap-3 mb-3`}>
                        ...
                        <TouchableOpacity style={tw`rounded-full bg-[#22c55e] px-4 py-2`} onPress={() => onViewDetails(project)}>
                            ...
                            <Text style={tw`text-xs font-semibold text-white`}>{t("Auto.Common.ViewDetails", "View Details")}</Text>
                        </TouchableOpacity>
                    </View>

                    <View style={tw`flex-col gap-3 mt-3`}>
                        <View style={tw`flex-row items-center`}>
                            <Text style={tw`font-semibold text-[#111827] w-28 text-sm`}>{t("Auto.Common.Duration", "Duration")}</Text>
                        ...
                        <View style={tw`flex-row items-center`}>
                            <Text style={tw`font-semibold text-[#111827] w-28 text-sm`}>{t("Auto.Common.TechStack", "Tech Stack")}</Text>
                            <Text style={tw`text-sm text-[#374151] flex-1`} numberOfLines={1}>{project.techStack}</Text>
                        </View>

                        <View style={tw`flex-row items-center`}>
                            <Text style={tw`font-semibold text-[#111827] w-28 text-sm`}>{t("Auto.Common.Team", "Team")}</Text>
                            <View style={tw`flex-row items-center`}>
                                {project.teamMembers.length > 0 ? project.teamMembers.slice(0, 4).map((member, i) => <MemberAvatar key={i} image={member.image} name={member.name} index={i} />) : <Text style={tw`text-gray-400 text-xs italic`}>{t("Auto.Common.Nomembers", "No members")}</Text>}
                            </View>
                        </View>

                        <View style={tw`flex-row items-center`}>
                            <Text style={tw`font-semibold text-[#111827] w-28 text-sm`}>{t("Auto.Common.Mentor", "Mentor")}</Text>
                            <View style={tw`flex-row items-center flex-wrap`}>
                                {project.mentors.length > 0 ? project.mentors.map((mentor, i) => <MemberAvatar key={i} image={mentor.image} name={mentor.name} index={i} />) : <Text style={tw`text-gray-400 text-xs italic`}>{t("Auto.Common.Nomentor", "No mentor")}</Text>}
                            </View>
                        </View>

                        <View style={tw`flex-row items-center`}>
                            <Text style={tw`font-semibold text-[#111827] w-28 text-sm`}>{t("Auto.Common.Marks", "Marks")}</Text>
  ...
  return <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
            <SafeAreaView style={tw`flex-1 bg-white`}>
                    <View style={tw`flex-row items-center justify-between p-6 pb-4 border-b border-gray-100`}>
                        <TouchableOpacity onPress={onClose} style={tw`flex-row items-center gap-2`}>
                            ...
                            <Text style={tw`font-semibold text-lg text-gray-800`}>{t("Auto.Common.Details", "Details")}</Text>
                        </TouchableOpacity>
          ...
          if (project.projectId !== null && onViewSubmissions) {
            onViewSubmissions(project);
                            ...
                            <Text style={tw`text-white text-sm font-semibold`}>{t("Auto.Common.Submissions", "Submissions")}</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    <ScrollView style={tw`flex-1 px-6 pt-4 pb-6`}>
                        ...
                        <View style={tw`mb-5`}>
                            <Text style={tw`text-lg font-semibold text-gray-900 mb-2`}>{t("Auto.Common.Domains", "Domain(s)")}</Text>
                            <View style={tw`flex-row flex-wrap gap-2`}>
                                {domains.length > 0 ? domains.map((d, i) => <View key={i} style={tw`px-3 py-1.5 rounded-full bg-[#16284F21]`}>
                                        ...
                                        </View>) : <Text style={tw`text-gray-400 text-sm italic`}>{t("Auto.Common.Nodomainsspecif", "No domains specified")}</Text>}
                            </View>
                        </View>

                        <View style={tw`mb-5`}>
                            <Text style={tw`text-lg font-semibold text-gray-900 mb-2`}>{t("Auto.Common.Duration", "Duration")}</Text>
                        ...
                        <View style={tw`mb-5`}>
                            <Text style={tw`text-lg font-semibold text-gray-900 mb-2`}>{t("Auto.Attr.TeamMembers", "Team Members")}</Text>
                            <View style={tw`flex-row items-center flex-wrap`}>
                                {project.teamMembers.length > 0 ? project.teamMembers.map((member, i) => <MemberAvatar key={i} image={member.image} name={member.name} index={i} />) : <Text style={tw`text-gray-400 text-sm italic`}>{t("Auto.Common.Nomembersassign", "No members assigned")}</Text>}
                            </View>
                        </View>

                        <View style={tw`mb-5`}>
                            <Text style={tw`text-lg font-semibold text-gray-900 mb-2`}>{t("Auto.Common.Mentors", "Mentor(s)")}</Text>
              ...
              return <View key={i} style={tw`flex-row items-center gap-3`}>
                                            ...
                                            <View>
                                                ...
                                                <Text style={tw`text-xs text-gray-500`}>{t("Auto.Common.FacultyGuide", "Faculty / Guide")}</Text>
                                            </View>
                                        </View>;
            }) : <Text style={tw`text-gray-400 text-sm italic`}>{t("Auto.Common.Nomentorassigne", "No mentor assigned")}</Text>}
                            </View>
                        </View>

                        <View style={tw`mb-5`}>
                            <Text style={tw`text-lg font-semibold text-gray-900 mb-2`}>{t("Auto.Common.Marks", "Marks")}</Text>
                            <View style={tw`self-start px-4 py-1.5 rounded-full bg-green-50`}>
                                <Text style={tw`text-green-700 text-sm font-semibold`}>{project.marks}{t("Auto.Common.pts", "pts")}</Text>
                            </View>
                        </View>

                        <View style={tw`mb-6`}>
                            <Text style={tw`text-lg font-semibold text-gray-900 mb-2`}>{t("Auto.Common.Attachments", "Attachments")}</Text>
