...
export default function StudentSubmissions({ projectId, projectTitle }: StudentSubmissionsProps) {
    ...
    return (
        <View style={tw`flex-1 p-4`}>
            <View style={tw`mb-4`}>
                ...
                <Text style={tw`text-gray-500 text-sm mt-1`}>{submissions.length} Total Submissions</Text>
            </View>
            ...
            {isLoading ? (
                ...
                <View style={tw`flex-1 justify-center items-center py-10`}>
                    <Text style={tw`text-gray-500`}>No submissions found.</Text>
                </View>
            ) : (
                <ScrollView style={tw`flex-1`}>
                    {submissions.map((sub) => (
                        <View key={sub.id} style={tw`flex-row items-center justify-between p-4 bg-white rounded-xl shadow-sm mb-3 border border-gray-100`}>
                            <View style={tw`flex-row items-center flex-1`}>
                                <Image
                                    source={{ uri: sub.photo || "https://ui-avatars.com/api/?name=Student" }}
                                ...
                                <View style={tw`flex-1 pr-2`}>
                                    ...
                                    <Text style={tw`text-xs text-gray-400 mt-0.5`}>Submitted: {sub.date}</Text>
                                </View>
                            </View>
                            {sub.fileUrl && (
                                <TouchableOpacity 
                                    style={tw`bg-green-50 px-3 py-1.5 rounded-full border border-green-200`}
                                    onPress={() => Linking.openURL(sub.fileUrl)}
                                >
                                    <Text style={tw`text-green-700 text-sm font-semibold`}>View File</Text>
                                </TouchableOpacity>
                            )}
                        </View>
                    ))}
