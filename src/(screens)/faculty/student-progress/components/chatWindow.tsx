...
import { View, Text, Modal, TouchableOpacity, Image, ScrollView, TextInput, KeyboardAvoidingView, Platform } from "react-native";
...
export default function ChatWindow({ parent, onClose, visible }: ChatWindowProps) {
  const messages = [
    { text: "Good morning Sir , I wanted to ask about her attendance.", isSender: false },
    { text: "Good morning. I'll share her attendance details.", isSender: true },
    ...
    { text: "Could you please update me on her recent performance?", isSender: false },
    ...
    <Modal visible={visible} animationType="slide" transparent>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={tw`flex-1 bg-black/50 justify-end`}>
        <View style={tw`bg-white rounded-t-3xl h-[85%] p-4 md:p-6 shadow-sm flex-col`}>
          <TouchableOpacity
            style={tw`flex-row items-center gap-2 md:gap-3 mb-4 md:mb-6`}
            onPress={onClose}
          >
            <CaretLeft size={28} color="#333" weight="bold" />
            <Text style={tw`text-lg md:text-2xl font-bold text-[#333]`}>Parent's Chat</Text>
          </TouchableOpacity>
          ...
          <ScrollView style={tw`flex-1 mb-4`} showsVerticalScrollIndicator={false}>
            {messages.map((msg, idx) => (
              <View
                key={idx}
                style={tw`flex-row w-full mb-4 ${msg.isSender ? "justify-end" : "justify-start"}`}
              >
                <View
                  style={tw`max-w-[80%] p-3 md:p-4 rounded-xl shadow-sm ${
                    msg.isSender
                      ? "bg-[#95D078] rounded-br-sm"
                      : "bg-[#EFF8E9] rounded-bl-sm"
                  }`}
                >
                  ...
                  <Text
                    style={tw`text-[10px] md:text-[11px] mt-1.5 text-right ${
          ...
          <View style={tw`bg-[#E4F6E6] rounded-full px-2 py-2 flex-row items-center gap-2 mb-4`}>
            ...
            <TextInput
              ...
              style={tw`flex-1 text-[13px] md:text-[15px] text-[#333] font-medium py-2`}
            />
