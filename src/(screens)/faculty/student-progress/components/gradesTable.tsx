...
export default function GradesTable({ grades }: GradesTableProps) {const { t } = useTranslation();
  ...
  return (
    <View style={tw`bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 w-full h-full flex-col min-h-[300px]`}>
      <Text style={tw`text-[#333333] text-[16px] md:text-xl font-bold mb-4 md:mb-6`}>{t("Auto.Common.Grades", "Grades")}
      ...
      <View style={tw`w-full flex-1`}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={tw`flex-1`}>
            <View style={tw`w-full min-w-[320px]`}>
              ...
              <View style={tw`flex-row border-b border-gray-100 pb-2`}>
                <Text style={tw`flex-1 text-[12px] md:text-sm font-medium text-[#8E8E8E]`}>{t("Auto.Common.Subject", "Subject")}</Text>
                <Text style={tw`w-16 text-[12px] md:text-sm font-medium text-[#8E8E8E]`}>{t("Auto.Common.Grade", "Grade")}</Text>
                <Text style={tw`w-24 text-[12px] md:text-sm font-medium text-[#8E8E8E] text-right`}>{t("Auto.Common.Improvement", "Improvement")}</Text>
              </View>
        ...
        <View style={tw`flex-1 items-center justify-center py-8`}>
            <Text style={tw`text-[13px] md:text-sm text-[#6B7280] text-center`}>{t("Auto.Common.Nogradesavailab", "No grades available for this student.")}
