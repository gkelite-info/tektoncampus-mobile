...
import { View, TouchableOpacity, ScrollView, Switch, ActivityIndicator, Linking } from 'react-native';
...
export default function LinkedAccountsScreen() {const { t } = useTranslation();
  ...
  const loadFromSession = async (user: any) => {
    ...
    prev.map((acc) => ({
      ...
      description: activeProviders.includes(acc.id) ? "Connected" : "Not Connected"
    }))
    );
    ...
    if (userId) {
      ...
      prev.map((acc) => {
        ...
        return {
          ...
          description: isConnected ? `Connected to ${dbRecord?.email || "your account"}` : "Not Connected"
        };
      })
      );
    }
  ...
  const handleToggleLinkedAccount = async (accountId: string) => {
    ...
    if (targetAccount.connected) {
      try {
        ...
        setAccounts((prev) =>
        prev.map((a) => a.id === accountId ? { ...a, connected: false, description: "Not Connected" } : a)
        );
        Toast.show({ type: "success", text1: `${targetAccount.name} disconnected!` });
      } catch (error: any) {
        Toast.show({ type: "error", text1: error.message || "Failed to disconnect." });
      } finally {
      ...
      try {
        // WIP Note: In a real mobile app, this needs Expo Auth Session or Linking callbacks
        const redirectUrl = `tektoncampus://settings/linked-accounts`;
        await linkUserIdentity(accountId, redirectUrl);
      } catch (error: any) {
        Toast.hide();
        Toast.show({ type: "error", text1: error.message || "Failed to initialize connection." });
        setProcessingId(null);
      }
    }
  };
