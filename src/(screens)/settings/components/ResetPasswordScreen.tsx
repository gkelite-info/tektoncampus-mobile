...
export default function ResetPasswordScreen() {const { t } = useTranslation();
  ...
  const handleUpdate = async () => {
    if (!newPwd || !confirmPwd) {
      Toast.show({ type: "error", text1: "Please fill out both password fields." });
      return;
    }
    ...
    if (!isValidLength || !hasUppercase || !hasNumber || !hasSpecial) {
      Toast.show({ type: "error", text1: "Please ensure the password meets all requirements." });
      return;
    }

    setIsProcessing(true);
    Toast.show({ type: "info", text1: "Updating password...", autoHide: false });
