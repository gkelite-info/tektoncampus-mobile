...
export default function VerifyPasswordScreen() {const { t } = useTranslation();
  ...
  const handleContinue = async () => {
    ...
    Toast.show({ type: "info", text1: "Verifying password...", autoHide: false });

    try {
      await verifyCurrentPassword(email, password);
      Toast.hide();
      Toast.show({ type: "success", text1: "Password verified!" });
      navigation.replace("ResetPassword");
    } catch (error: any) {
      Toast.hide();
      Toast.show({ type: "error", text1: "Incorrect password. Please try again." });
    } finally {
  ...
  const handleForgotPassword = async () => {
    ...
    Toast.show({ type: "info", text1: "Sending reset link...", autoHide: false });

    try {
      await sendPasswordResetEmail(email);
      Toast.hide();
      Toast.show({ type: "success", text1: "Password reset link sent to your email!" });
    } catch (error: any) {
      Toast.hide();
