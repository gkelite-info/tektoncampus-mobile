...
import { ArrowSquareDown, CheckCircle, FilePdf, X } from "phosphor-react-native";
...
function ModalShimmer() {
  return <View className="flex-col flex-1 opacity-60">
            <View className="flex-col gap-4 p-5 flex-1">
                ...
                <View className="flex-col gap-2 mt-4">
                    ...
                    {Array.from({
          length: 3
        }).map((_, index) => <View key={`student-resource-shimmer-${index}`} className="flex-row items-center justify-between gap-3 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
...
export function TopicPdfViewModal({
  ...
  useEffect(() => {
    ...
    async function loadResources() {
      ...
      try {
        ...
        if (!cancelled) {
          setResources(data);
          setSelectedResourceIds(data.length > 0 ? [data[0].collegeSubjectUnitTopicResourceId] : []);
        }
      } catch (err: any) {
        if (!cancelled) {
  ...
  const selectedResources = resources.filter(resource => selectedResourceIds.includes(resource.collegeSubjectUnitTopicResourceId));
  return <Modal visible={isOpen} transparent animationType="fade" onRequestClose={onClose}>
            <View className="flex-1 bg-black/40 justify-center items-center px-4" style={{
      backgroundColor: "rgba(0,0,0,0.4)"
                ...
                <View className="bg-white rounded-2xl w-full max-w-sm flex-col max-h-[80%] relative overflow-hidden shadow-xl">
                    ...
                    {loadingResources ? <ModalShimmer /> : <>
                            <ScrollView className="p-5 flex-col" showsVerticalScrollIndicator={false}>
                                <Text className="text-base text-gray-800 mb-4 pr-6 flex-row flex-wrap" style={{
              fontFamily: fonts.bold
                ...
                return <TouchableOpacity key={resource.collegeSubjectUnitTopicResourceId} onPress={() => toggleSelectedResource(resource.collegeSubjectUnitTopicResourceId)} className={`flex-row items-center justify-between gap-3 rounded-xl px-3 py-2.5 border my-1 ${isSelected ? "bg-[#F0FBF5] border-[#43C17A]" : "bg-gray-50 border-gray-100"}`}>
                                                    <View className="flex-row items-center gap-3 flex-1">
                                                        <CheckCircle size={18} color={isSelected ? "#43C17A" : "#D1D5DB"} weight={isSelected ? "fill" : "regular"} />
                                                        <View className="bg-red-100 rounded-lg p-1.5">
                                                            <FilePdf size={18} color="#EF4444" weight="duotone" />
                                                        </View>
                                                        <View className="flex-1">
                                                            <Text className="text-xs text-gray-700" numberOfLines={1} style={{
                        fontFamily: fonts.semiBold
                      }}>
                                                                {resource.resourceName}
                                                            </Text>
                                                            <Text className="text-[10px] text-gray-400 capitalize" style={{
                        fontFamily: fonts.regular
                                                    ...
                                                    {isDownloading && isSelected && <View className="flex-row items-center gap-1">
            ...
            }} disabled={selectedResources.length === 0 || downloadingId !== null} className={`flex-1 py-2.5 rounded-xl flex-row items-center justify-center gap-2 ${selectedResources.length === 0 || downloadingId !== null ? "bg-[#43C17A]/50" : "bg-[#43C17A]"}`}>
                                    {downloadingId !== null ? <>
