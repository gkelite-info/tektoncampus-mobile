                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} className="w-full flex-row py-2">
                    <View className="flex-row items-center gap-2 mr-4">
                        ...
                        <TouchableOpacity
                            ...
                            className="px-3 py-1 bg-[#DCEAE2] rounded-full flex-row items-center justify-between"
                            style={{ maxWidth: 150 }}
                        >
                            <Text className="text-[#43C17A] text-base truncate" numberOfLines={1} style={{ fontFamily: fonts.medium }}>
                        ...
                        <View className="flex-row items-center gap-2 mr-4">
                            ...
                            <View className="px-3 py-1 bg-[#DCEAE2] rounded-full">
                                <Text className="text-[#43C17A] text-base font-medium">
                                    Sem {subjectProps[0]?.semester || "N/A"}
                                </Text>
                            </View>
                        </View>
                    )}
                ...
                {filteredSubjects.length > 0 ? (
                    filteredSubjects.map((item, index) => {
                        ...
                        return (
                            <View
                                key={index}
                                className="bg-white rounded-lg w-full min-h-[230px] p-4 flex-col justify-between shadow-sm border border-gray-100"
                            >
                                <View className="flex-col gap-2">
                                    <View className="flex-row items-center justify-between gap-2">
                                        <View className="flex-row items-center gap-3 flex-1 min-w-0">
                                            <Text className="text-[#282828] text-base max-w-[60%] truncate" numberOfLines={1} style={{ fontFamily: fonts.bold }}>
                                                {item.subjectTitle}
                                            </Text>
                                        ...
                                        <TouchableOpacity
                                            className="bg-[#7051E1] px-2.5 py-1.5 rounded-md"
                                            onPress={() => handleViewDetails(item.subjectTitle)}
                                        >
                                    ...
                                    <View className="flex-col gap-2 mt-1">
                                        <View className="flex-row items-center gap-2">
                                            ...
                                            <Avatar
                                                ...
                                                alt="faculty"
                                            />
                                            <Text className="text-[#525252] text-base" style={{ fontFamily: fonts.regular }}>
                                                {item.lecturer}
                                            </Text>
                                        </View>
                                        ...
                                        <Text className="text-[#525252] text-base truncate" numberOfLines={1} style={{ fontFamily: fonts.regular }}>
                                            <Text className="text-[#282828]" style={{ fontFamily: fonts.medium }}>{t("Next lesson :")} </Text>
                                            {item.nextLesson}
                                        </Text>
                                    </View>
                                </View>
                                ...
                                <View className="flex-col mt-2 w-full">
                                    <View className="w-full rounded-full h-2 bg-gray-200 mt-3 relative overflow-visible">
                                        <View
                                            className="absolute top-0 left-0 h-full bg-[#6D4EE0] rounded-full"
                                            style={{ width: `${percentage}%` }}
                                        />
                                        {percentage > 0 && (
                                            <View
                                                className="absolute top-1/2 bg-white rounded-full border border-gray-300 shadow-sm"
                                                style={{
                                                    left: `${percentage}%`,
                                                    transform: [{ translateX: -ballSize / 2 }, { translateY: -ballSize / 2 }],
                                                    height: ballSize,
                                                    width: ballSize,
                                                }}
                                            />
                                    ...
                                    <View className="w-full h-5 mt-1 relative">
                                        ...
                                        >
                                            <Text className="text-[#6D4EE0] text-base" style={{ fontFamily: fonts.bold }}>
                    ...
                    <View className="py-10 flex-row justify-center items-center">
                        <Text className="text-gray-400 text-sm" style={{ fontFamily: fonts.regular }}>
                    ...
                    <View className="bg-white rounded-lg w-[80%] max-h-[50%] p-4 shadow-xl">
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                ...
                                >
                                    <Text className={`text-sm ${selectedSubject === item ? "text-[#43C17A]" : "text-gray-700"}`} style={{ fontFamily: fonts.bold }}>
                                        {item === "All" ? t("All") : item}
                                    </Text>
                                </TouchableOpacity>
                            )}
