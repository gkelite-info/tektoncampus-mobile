            ...
            <View className="flex-row flex-wrap gap-2">
                <View className="flex-row items-center gap-2 mr-2">
                    ...
                    <View className="px-3 py-1 bg-[#DCEAE2] rounded-full">
                        <Text className="text-[#43C17A] text-base" style={{ fontFamily: fonts.medium }}>
...
function UnitCard({ unit, onOpenTopicPdf }: UnitCardProps) {
    ...
    const renderTopicList = (topics: UnitTopic[]) => {
        if (topics.length === 0) {
            return <Text className="text-gray-400 text-xs" style={{ fontFamily: fonts.italic }}>{t("No topics found")}</Text>;
        }
        return topics.map((topic: UnitTopic) => (
            <View key={topic.topicId} className="flex-row items-start justify-between gap-2 py-1">
                <View className="flex-row items-start gap-2 flex-1">
                    <CheckCircle
                        ...
                        style={{ marginTop: 2 }}
                    />
                    <Text className={`text-xs text-[#3F3F3F] flex-1 ${topic.isCompleted ? "text-gray-400 line-through" : ""}`} style={{ fontFamily: fonts.regular }}>
                        {topic.name}
                    </Text>
                </View>

                <TouchableOpacity
                    onPress={() => onOpenTopicPdf({
    ...
    return (
        <View className={`rounded-2xl p-4 ${colors.cardBg} w-full mb-4`}>
            <TouchableOpacity
            ...
            <View className="bg-white rounded-xl p-3 flex-col">
                <Text className={`text-base mb-2 ${colors.title}`} style={{ fontFamily: fonts.bold }}>
                    {unit.title}
                </Text>
...
export function SubjectDetailsCard({
    ...
    return (
        <ScrollView className="w-full p-1 bg-[#F5F5F7]" showsVerticalScrollIndicator={false}>
            <TouchableOpacity
                onPress={onBack}
                className="mb-4 flex-row items-center gap-2"
            >
                <ArrowLeft size={16} color="#7153E1" weight="bold" />
                <Text className="text-[#7153E1] text-sm" style={{ fontFamily: fonts.bold }}>
                    {t("Go Back")}
                </Text>
            </TouchableOpacity>
            ...
            <View className="flex-col gap-3 mb-4">
                ...
                <View className="flex-row items-center gap-2 bg-[#E8ECF3] px-3 py-1.5 rounded-full self-start">
                    ...
                    <Text className="text-[#4C4C4C] text-base" style={{ fontFamily: fonts.medium }}>
                        {t("Faculty :")}{" "}
                    </Text>
                    <Text className="text-[#122A5E] text-base" style={{ fontFamily: fonts.bold }}>
                        {details.lecturer}
                    </Text>
                </View>
            </View>
            ...
            <View className="mb-4 flex-row items-center gap-3 rounded-xl border border-[#BBF7D0] bg-[#ECFDF5] p-3 shadow-sm">
                ...
                <View className="flex-1">
                    <Text className="text-base text-[#14532D]" style={{ fontFamily: fonts.bold }}>
                        AI Generated Notes
                    </Text>
                    <Text className="text-sm text-[#4B5563] mt-0.5" style={{ fontFamily: fonts.medium }}>
            ...
            <View className="flex-col w-full mb-10">
                ...
                {dynamicUnits.length === 0 && (
                    <Text className="text-gray-500 p-4 text-center text-base" style={{ fontFamily: fonts.italic }}>
                        {t("No units found")}
                    </Text>
                )}
            </View>
