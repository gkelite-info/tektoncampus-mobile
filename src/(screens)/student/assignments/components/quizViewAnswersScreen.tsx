...
interface QuizViewAnswersProps {
    ...
    onBack: (updatedParams: { modal: string }) => void;
}

function QuizViewAnswersScreenContent({ quiz, routeParams, onBack }: QuizViewAnswersProps) {
    const { quizId: activeQuizId, submissionId } = routeParams;
                ...
                <View className="flex-row justify-between items-start mb-4 pt-2">
                    <View className="flex-row items-center gap-2 flex-1 mr-4">
                        ...
                        <View className="flex-1">
                            ...
                            {quiz?.topic ? (
                                <Text numberOfLines={1} className="text-xs font-medium text-[#282828]">
                                    {quiz.topic}
                                </Text>
                            ) : null}
                        </View>
                            ...
                            return (
                                ...
                                >
                                    <View className="flex-row justify-between items-start mb-4 gap-2">
                                        <Text className="text-base font-bold text-[#282828] flex-1">
                                            Q{index + 1}. {q.questionText}
                                        </Text>
                                        <View
                                            className={`px-3 py-1 rounded-md ${isCorrect ? "bg-[#43C17A]" : "bg-[#FF3B30]"
                                                }`}
                                        >
                                    ...
                                    {q.questionType === "Multiple Choice" ? (
                                            ...
                                            {q.quiz_question_options
                                                ...
                                                .map((opt: any) => {
                                                    ...
                                                    return (
                                                        <View
                                                            key={opt.optionId}
                                                            className={`flex-row items-center gap-3 px-3 py-2 rounded-md ${bgClass}`}
                                                        >
                                        ...
                                        <View className="gap-2">
                                            <View
                                                className={`flex-row items-center gap-3 px-3 py-2 rounded-md ${isCorrect ? "bg-[#d1f4e0]" : "bg-[#fce8e6]"
                                                    }`}
                                            >
                                                <View className="w-5 h-5 items-center justify-center shrink-0">
                                                    {isCorrect ? (
                                                        <CheckCircle size={20} weight="fill" color="#43C17A" />
                                                    ) : (
                                                        <XCircle size={20} weight="fill" color="#FF3B30" />
                                                    )
                                                    }
                                                </View>
                                                <Text
                                                    className={`text-sm flex-1 ${isCorrect ? "text-[#43C17A]" : "text-[#FF3B30]"
                                                        }`}
                                                >
                                                    {t("Your answer:")}{" "}
...
export default function QuizViewAnswersScreen({ quiz, routeParams, onBack }: QuizViewAnswersProps) {
    return (
        <Suspense
            fallback={
