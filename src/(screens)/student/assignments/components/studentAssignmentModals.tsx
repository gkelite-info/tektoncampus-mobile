...
import { Modal, View, TouchableOpacity, ActivityIndicator, StyleSheet, Platform } from 'react-native';
...
import {
  ...
  deleteAssignmentSubmission } from
"@/lib/helpers/student/assignments/insertAssignmentSubmission";
...
export function StudentAssignmentUploadModal({
  ...
  existingFilePath,
  onUploadSuccess
}: UploadModalProps) {const { t } = useTranslation();
...
}: DetailsModalProps) {const { t } = useTranslation();
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            <View className="flex-1 justify-center items-center bg-black/40 p-4">
                <TouchableOpacity activeOpacity={1} onPress={onClose} className="absolute inset-0 w-full h-full" />
