...
interface StudentDiscussionCardProps {
    ...
    isCompleted?: boolean;
    uploadedFiles?: Array<{ studentDiscussionUploadId: number; fileUrl: string }>;
    ...
    const handleRemoveUpload = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                Toast.show({ type: "success", text1: t("File removed successfully") });
            }
        } catch (error) {
            Toast.show({ type: "error", text1: t("An error occurred") });
        } finally {
            setIsDeleting(false);
                ...
                <View className="mt-1">
                    ...
                    >
                        ...
                        {uploadedFiles.map((file, idx) => (
                            <View
                                key={`stu-m-${idx}`}
                                className="flex-row items-center bg-[#e2f6ea] px-2.5 py-1 rounded border border-[#43C17A]/30 mr-2"
                            >
                                <TouchableOpacity
                                    onPress={() => handleOpenFile(file.fileUrl)}
                                ...
                                {!isCompleted && (
                                    <TouchableOpacity
                                        onPress={() => setDeleteUploadId(file.studentDiscussionUploadId)}
                                    >
