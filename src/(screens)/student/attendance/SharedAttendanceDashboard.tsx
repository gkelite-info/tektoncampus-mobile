...
import { View, ScrollView, TouchableOpacity, Platform, LayoutAnimation, UIManager, Dimensions, RefreshControl } from 'react-native';
...
function StatusBadge({
  ...
  if (status === "CLASS_CANCEL") {
    return <View className="bg-[#F3F4F6] px-[10px] py-[2px] rounded-full">
      <Text className="text-[#6B7280] text-[12px] font-semibold">
        {t(label)}
      </Text>
    </View>;
  }
  return <Text className={`text-[13px] ${getStatusTextClass(status)}`}>
    {t(label)}
  </Text>;
}
export default function SharedAttendanceDashboard({ targetUserId }: { targetUserId: number }) {
  const t = useTranslations("Attendance_module.student");
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const data = await getStudentDashboardData(userId!, `${year}-${month}-${day}`, currentPage, rowsPerPage, isInter);
        if (isMounted) {
  ...
  const tableRows: TableRow[] = dashboardData?.tableData?.map(row => ({
    Subject: row.subject,
    Faculty: row.faculty,
  ...
  return <SafeAreaView edges={["left", "right", "bottom"]} className="flex-1 bg-[#F5F5F5]">
    <ScrollView className="flex-1 bg-[#f4f5f6] " showsVerticalScrollIndicator={false} contentContainerStyle={{
      paddingBottom: 140,
      ...
      <View className="px-4 pt-4">
        {!showSubjectAttendanceTable && !showSubjectAttendanceDetails && <>
          <View className="mb-4">
            <Text className="text-[#282828] text-[22px] mb-1" style={{
              fontFamily: fonts.bold
            }}>
              {t("Attendance")}
            </Text>
            <Text className="text-gray-600 text-[13px]" style={{
          ...
          {dataLoading ? <DashboardSkeleton /> : <View className="flex-col gap-3 w-full">
          ...
          <View className="mt-3">
            <AiAttendanceNotificationBanner className="min-h-[70px] py-4" message={dashboardData?.attendancePolicyInsight?.message || "Attendance insight will appear once records are available."} />

          </View>

          <View className="mt-3 p-3">
            <Text className="text-[#282828] text-[17px]" style={{
              fontFamily: fonts.semiBold
            }}>
              {isToday ? t("Today's Attendance") : t("Attendance – {date}", {
                date: formattedDate
              })}
            </Text>
            <Text className="text-gray-500 text-[13px] mt-0.5" style={{
              fontFamily: fonts.regular
            }}>
              {t("Classes on {date}", {
                date: formattedDate
            ...
            {dataLoading ? <View className="mt-5">
              <TableSkeleton />
            </View> : <>
              <View className="flex-col mt-3 w-full">
                {tableRows.map((row, i) => {
                  ...
                  return <View key={i} className={`overflow-hidden${!isLast ? " border-b border-gray-100" : ""}`}>

                    <TouchableOpacity className="py-3 flex-row justify-between items-center" activeOpacity={0.7} onPress={() => toggleRow(i)}>

                      <View className="flex-1 flex-col gap-0.5">
                        <Text className="text-[#515151] text-[11px]" style={{
                          fontFamily: fonts.regular
                        }}>
                          {t("Subject Name")}
                        </Text>
                        <Text className="text-[14px] text-[#282828] pr-2" numberOfLines={1} style={{
                      ...
                      <View className="flex-row items-center gap-2">
                        <View className="w-8 h-8 rounded-full bg-blue-50 items-center justify-center">
                          <Text className="text-blue-500 text-[9px]" style={{
                            fontFamily: fonts.bold
                          }}>{t("PDF")}
                        ...
                        <View className="w-6 h-6 rounded-full bg-[#43C17A] items-center justify-center">
                          <CaretDown size={14} color="#FFFFFF" weight="bold" style={{
                            transform: [{
                    ...
                    {isExpanded && <View className="pb-3 flex-col gap-2.5 px-2 pt-1 rounded-lg mb-1">
                      <View className="flex-row justify-between items-center">
                        <Text className="text-[#282828] text-[13px]" style={{
                          fontFamily: fonts.medium
                        }}>
                          {t("Faculty")}
                        </Text>
              ...
              {totalPages > 1 && <View className="flex-row justify-end items-center gap-2 mt-6 mb-4 w-full">
                <TouchableOpacity onPress={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className={`w-10 h-10 items-center justify-center rounded-lg border ${currentPage === 1 ? "border-gray-200" : "border-gray-300 bg-white"}`}>
                ...
                {[...Array(totalPages)].map((_, i) => <TouchableOpacity key={i} onPress={() => setCurrentPage(i + 1)} className={`w-8 h-8 rounded-lg items-center justify-center ${currentPage === i + 1 ? "bg-[#16284F]" : "border border-gray-300 bg-white"}`}>
                ...
                <TouchableOpacity onPress={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className={`w-10 h-10 items-center justify-center rounded-lg border ${currentPage === totalPages ? "border-gray-200" : "border-gray-300 bg-white"}`}>
              ...
              {tableRows.length === 0 && <View className="mt-4 border border-gray-200 p-4 rounded-lg bg-white items-center">
                <Text className="text-gray-400 text-[13px] text-center" style={{ fontFamily: fonts.italic }}>
        ...
        {showSubjectAttendanceTable && <SubjectAttendance onBack={() => setCurrentTab(null)} onNavigate={(screen, params) => {
        ...
        {showSubjectAttendanceDetails && <SubjectAttendanceDetailsClient route={{
          params: {
            subjectId: selectedSubjectId
          }
        }} navigation={{
          goBack: () => setCurrentTab("subject-attendance")
        }} />}

        {!hideRightSection && Platform.OS === "web" && <View className="w-[32%] flex-col gap-3 p-2 pt-0">
          <CourseScheduleCard />
          <WorkWeekCalendar activeDate={viewDate} onDateSelect={setViewDate} />
          <View className="mt-5">
            <AttendanceInsight weeklyData={dashboardData?.weeklyData || [0, 0, 0, 0, 0, 0, 0]} />
