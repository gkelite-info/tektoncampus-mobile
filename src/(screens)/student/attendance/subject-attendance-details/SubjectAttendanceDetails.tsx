...
const StatusBadge = ({
  ...
  return <View className="w-[85px] h-[26px] items-center justify-center rounded-lg" style={{
    backgroundColor: bg
  }}>
            <Text style={{
      color: color,
...
export default function SubjectAttendanceDetailsClient({
  ...
  const getStudentAttendanceDetails = async (...args: any[]): Promise<any> => ({
    ...
    attendancePolicyInsight: {
      message: "Looking good! Keep this consistency up to secure your terms."
    }
  });
  useEffect(() => {
  ...
  const cards: CardItem[] = [{
    id: 1,
    icon: <Chalkboard size={24} color="#714EF2" weight="fill" />,
    value: data?.headerStats.total ?? 0,
    label: t("Total Classes"),
    style: "bg-[#E2DAFF]"
  }, {
    id: 2,
    icon: <Chalkboard size={24} color="#FFBC72" weight="fill" />,
    value: data?.headerStats.attended ?? 0,
    label: t("Attended"),
    style: "bg-[#FFEDDA]"
  }, {
    id: 3,
    icon: <Chalkboard size={24} color="#F62D2D" weight="fill" />,
    value: data?.headerStats.leave ?? 0,
    label: t("Leave"),
    style: "bg-[#FFE0E0]"
  }, {
    id: 4,
    icon: <Percent size={24} color="#60AEFF" weight="fill" />,
  ...
  return <View className="flex-1">
          ...
          fontFamily: fonts.regular
            ...
            <View className="w-full mt-4 flex-row flex-wrap justify-between gap-y-3">
                {cards.map((card, index) => <TouchableOpacity key={index} onPress={() => handleCardPress(card.id)} style={{
        width: isDesktopView ? "23%" : "48%"
                        ...
                        <View className="mt-4">
                            <Text className="text-xl text-[#282828]" style={{
            fontFamily: fonts.bold
          }}>{card.value}</Text>
            ...
            <View className="mt-4 w-full bg-indigo-50/60 border border-indigo-100 p-3 rounded-xl">
                <Text className="text-indigo-900 text-sm" style={{
        ...
        fontFamily: fonts.semiBold
      }}>{t("Subject Detail View")}</Text>

                <View className="flex-row flex-wrap items-center gap-2 mt-2">
                    <View className="flex-row items-center bg-gray-50 px-2.5 py-1.5 rounded-lg border border-gray-100">
                        <Text className="text-gray-500 text-base mr-1" style={{
            fontFamily: fonts.medium
          }}>{t("Subject:")}</Text>
                        <Text className="text-emerald-700 text-base" style={{
            fontFamily: fonts.semiBold
            ...
            <View className="mt-5 w-full">
                <View className="flex-row bg-gray-100 py-2.5 px-3 rounded-t-lg border-b border-gray-200">
                    <Text className="flex-1 text-gray-600 text-base" style={{
          fontFamily: fonts.bold
        }}>{t("Date")}</Text>
                    <Text className="flex-1 text-gray-600 text-base text-center" style={{
          fontFamily: fonts.bold
        }}>{t("Time")}</Text>
                    <Text className="flex-1 text-gray-600 text-base text-right pr-2" style={{
          fontFamily: fonts.bold
        }}>{t("Status")}</Text>
                </View>

                {loading ? <View className="py-8 items-center justify-center">
                        <ActivityIndicator size="small" color="#16284F" />
                    </View> : data?.rows.map((row: any, idx: number) => <View key={idx} className="flex-row items-center py-3 px-3 border-b border-gray-100 bg-white">
                            <Text className="flex-1 text-gray-800 text-base" style={{
          fontFamily: fonts.medium
        }}>{row.date}</Text>
                            <Text className="flex-1 text-gray-500 text-base text-center" style={{
          fontFamily: fonts.regular
        }}>{row.time}</Text>
