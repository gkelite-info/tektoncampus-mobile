...
const getStudentDashboardData = async (...args: any[]): Promise<any> => ({
  ...
  attendancePolicyInsight: {
    message: "Maintain above 75% to stay eligible for exams."
  }
});
const CardComponent = ({
  label,
  ...
  style,
  icon
}: any) => <View className={`p-4 rounded-xl shadow-sm bg-amber-50 mb-2 ${style}`}>
        {icon}
        <Text className="text-xl mt-2 text-[#282828]" style={{
    fontFamily: fonts.bold
  }}>{value}</Text>
        {totalPercentage && <Text className="text-md text-emerald-600" style={{
...
const WorkWeekCalendar = ({
  ...
  return <View className={`bg-gray-50 p-4 rounded-xl ${style}`}><Text>{t("Auto.Common.CalendarSection", "Calendar Section")}</Text></View>;
};
const AiAttendanceNotificationBanner = ({
  message
}: any) => <View className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl"><Text className="text-indigo-900 text-xs font-medium">{message}</Text></View>;
const TableComponent = ({
  ...
  return <View className="p-4 bg-gray-50 rounded-xl"><Text>{t("Auto.Common.DesktopDataView", "Desktop Data View Grid")}</Text></View>;
};
interface CardItem {
  id: number;
            ...
            <View className="flex-row justify-between items-center w-full">
                <View className="flex-col flex-1 pr-2">
                    <View className="flex-row items-center gap-1">
                        ...
                        <Text className="text-[#282828] text-2xl" style={{
            fontFamily: fonts.bold
          }}>{t("Attendance")}</Text>
                    </View>
                    <Text className="text-gray-500 text-sm mt-0.5" style={{
          fontFamily: fonts.regular
        }}>
                ...
                {isDesktopView && <View className="w-[320px]">
            ...
            <View className={`w-full mt-4 gap-3 ${isDesktopView ? "flex-row items-start" : "flex-col"}`}>
                <View className={`gap-3 ${isDesktopView ? "flex-row flex-1" : "flex-row w-full justify-between"}`}>
                    {cards.map((card, index) => <TouchableOpacity key={index} className="flex-1" onPress={() => card.id === 1 && router.navigate("AttendanceRoot")}>
            
                            <CardComponent style={card.style} icon={card.icon} value={card.value} label={card.label} totalPercentage={card.totalPercentage} />
            ...
            <View className="my-3 w-full">
                <AiAttendanceNotificationBanner message={dashboardData?.attendancePolicyInsight?.message || "Attendance insight will appear once records are available."} />
        
            </View>

            <View className="mt-2 flex-col items-start w-full">
                <Text className="text-[#282828] text-lg" style={{
        fontFamily: fonts.semiBold
      }}>{t("Subject-Wise Breakdown")}</Text>
                {isDesktopView ? <View className="w-full mt-2">
                        <TableComponent tableData={rawTableData} isLoading={loading} />
                    </View> : <View className="w-full flex-col mt-2">
                        {loading ? <View className="flex justify-center p-6 w-full">
                                <ActivityIndicator size="small" color="#16284F" />
                            </View> : rawTableData.map((row: any, i: number) => {
          
          const isExpanded = expandedRow === i;
          return <View key={i} className="border-b border-gray-100 w-full">
                                        <TouchableOpacity className="py-3 flex-row justify-between items-center w-full" onPress={() => setExpandedRow(isExpanded ? null : i)}>
                  
                                            <View className="flex-col flex-1 pr-2">
                                                <Text className="text-gray-400 text-base font-medium uppercase tracking-wider">{t("Subject Name")}</Text>
                                                <Text className="text-base text-[#282828] mt-0.5" numberOfLines={1} style={{
                  fontFamily: fonts.semiBold
                }}>
                                                    {row.subjectName}
                                                </Text>
                    ...
                    transform: [{
                      rotate: isExpanded ? "180deg" : "0deg"
                                        ...
                                        {isExpanded && <View className="pb-4 pt-1 px-1 flex-col gap-2 bg-gray-50/50 rounded-lg mb-2">
                                                {[{
                label: t("Total"),
                value: row.total
              }, {
                ...
                highlight: true
              }].map((item, idx) => <View key={idx} className="flex-row justify-between items-center py-0.5">
                  ...
                  fontFamily: fonts.medium
                                                ...
                                                <TouchableOpacity className="mt-2 items-end" onPress={() => (onNavigate || router.navigate)("SubjectDetails", {
                subjectId: row.subjectId
              })}>
                    
                                                    <Text className="text-indigo-600 underline text-base" style={{
                  fontFamily: fonts.semiBold
                }}>{t("View Details")}</Text>
                                                </TouchableOpacity>
                                            </View>}
                ...
                {totalPages > 1 && <View className="flex-row justify-end items-center gap-2 mt-6 mb-8 w-full">
                        <TouchableOpacity onPress={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className={`w-9 h-9 items-center justify-center rounded-lg border ${currentPage === 1 ? "border-gray-200" : "border-gray-300 bg-white"}`}>
            ...
            color: currentPage === 1 ? "#D1D5DB" : "#4B5563"
                        ...
                        {Array.from({
          length: totalPages
        }).map((_, i) => <TouchableOpacity key={i} onPress={() => setCurrentPage(i + 1)} className={`w-9 h-9 items-center justify-center rounded-lg ${currentPage === i + 1 ? "bg-[#16284F]" : "border border-gray-300 bg-white"}`}>
                        ...
                        <TouchableOpacity onPress={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className={`w-9 h-9 items-center justify-center rounded-lg border ${currentPage === totalPages ? "border-gray-200" : "border-gray-300 bg-white"}`}>
