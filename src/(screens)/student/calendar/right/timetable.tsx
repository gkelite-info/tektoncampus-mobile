...
export const Loader = () => <View className="flex-1 justify-center items-center h-[300px]">
...
export default function CalendarTimeTable({
  ...
  useEffect(() => {
    ...
    setTodayDay(now.toLocaleString("en-US", {
      weekday: "short"
    }));
  }, []);
  useEffect(() => {
    if (selectedDate) {
      ...
      setMobileDayName(dateObj.toLocaleDateString("en-US", {
        weekday: "short"
      }));
    }
    const loadTimetable = async () => {
      try {
        ...
        const {
          data: userRow
        } = await supabase.from("users").select("userId").eq("auth_id", user.id).single();
        ...
        const timetableWithResources = await Promise.all(rawData.map(async (item: any) => {
          let pdfUrl = null;
          if (item.topicId) {
  ...
  return <ScrollView className="flex-1 bg-gray-50/50 px-4" showsVerticalScrollIndicator={false}>
            ...
            <View className="flex-row items-center mt-3 mb-4">
                ...
                <View className="bg-gray-200/80 h-12 flex-1 justify-center px-4 rounded-r-xl">
                    <Text className="text-[#16284F] font-bold text-base">{t("Auto.Common.Timetable", "Timetable")}
            ...
            <View className="flex-col gap-3 pb-6">
                {loading ? <TimetableCardShimmer count={4} /> : timetable.length === 0 ? <View className="flex-col items-center justify-center py-12">
                        <Text className="text-center text-gray-500 font-medium text-sm">{t("Auto.Common.Noclassesschedu", "No classes scheduled")}

          </Text>
                    </View> : timetable.map((item, index) => {
        
        return <View key={index} className="bg-white rounded-xl p-4 flex-row gap-3 items-center shadow-sm border border-gray-100">
                            ...
                            <View className="flex-1 min-w-0 pr-6 relative">
                                ...
                                <Text className="text-gray-700 text-xs mt-1.5 truncate">
                                    <Text className="font-semibold text-gray-800">{t("Auto.Common.Topic", "Topic:")}</Text>
                                    {item.topic || "N/A"}
                                </Text>

                                <View className="flex-row items-center flex-wrap gap-x-2 mt-1">
                                    <Text className="text-gray-500 text-[11px]">
                                        <Text className="font-semibold text-gray-700">{t("Auto.Common.Room", "Room:")}</Text>
                                        {item.room || "-"}
                                    </Text>
                                    <Text className="text-gray-300 text-[11px]">·</Text>
                                    <Text className="text-gray-500 text-[11px] flex-1 truncate">
                                        <Text className="font-semibold text-gray-700">{t("Auto.Common.Faculty", "Faculty:")}</Text>
                                        {item.faculty}
                                    </Text>
                                </View>

                                {item.isCancelled && <Text className="text-red-500 text-[10px] font-black tracking-wide mt-1">{t("Auto.Common.CANCELLED", "CANCELLED")}

            </Text>}

                                <TouchableOpacity activeOpacity={0.7} onPress={() => item.pdfUrl && handleOpenPdf(item.pdfUrl)} disabled={!item.pdfUrl} className={`absolute right-[-6px] bottom-[6px] rounded-full h-8 w-8 items-center justify-center ${item.pdfUrl ? "bg-[#16284F]" : "bg-gray-200"}`}>
              
                                    <FilePdf size={16} color={item.pdfUrl ? "#FFFFFF" : "#9CA3AF"} weight="fill" />
