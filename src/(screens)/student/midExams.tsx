...
export default function MidExams({ onBack, translations }: MidExamsProps) {
    ...
    return (
        <ScrollView
            ...
            contentContainerStyle={{ padding: headerHeight + 16, paddingLeft: 10, paddingRight: 10 }}
        ...
        >
            <View className="flex-row items-center mb-1.5">
                ...
                <Text className="text-2xl text-[#282828]" style={{ fontFamily: fonts.semiBold }}>
                    {t("midTermTitle", "Mid Term Exam Enrollment")}
                </Text>
            </View>

            <Text className="text-sm text-[#515151] mb-4 pl-0.5 mt-1" style={{ fontFamily: fonts.regular }}>
            ...
            <View className="bg-white rounded-lg p-3.5 mb-3.5 shadow-sm border border-gray-100">
                {examMetaData.map((item, index) => (
                    <View className="flex-row items-center my-1.5" key={index}>
                        <View className="w-[35%]">
                            <Text className="text-base text-[#282828]" style={{ fontFamily: fonts.regular }}>{item.title}</Text>
                        </View>
                        <View className="bg-[#E5F6EC] rounded-full px-3 py-1">
                            <Text className="text-[#43C17A] text-sm" style={{ fontFamily: fonts.medium }}>{item.subTitle}</Text>
                        </View>
                    </View>
                ))}
                ...
                <View className="flex-row items-center my-1.5">
                    ...
                    <Text className="text-base text-[#282828] flex-1" style={{ fontFamily: fonts.regular }}>
            ...
            <View className="bg-white rounded-lg p-3.5 mb-3.5 shadow-sm border border-gray-100">
                <View className="flex-row justify-between items-center mb-4">
                    <Text className="text-base text-[#282828]" style={{ fontFamily: fonts.medium }}>
                        {t("selectSubjects", "Select Subjects to Enroll")}
                    </Text>
                    <View className="bg-[#E5F6EC] rounded-full px-3 py-1">
                        <Text className="text-[#43C17A] text-sm" style={{ fontFamily: fonts.medium }}>{t("hallTicket", "Hall Ticket")}</Text>
                    </View>
                </View>

                <View className="flex-row border-b border-gray-200 pb-2 mb-2.5">
                    <Text className="flex-[2.2] text-sm text-gray-400" style={{ fontFamily: fonts.bold }}>{t("subjectHeader", "Subject")}</Text>
                    <Text className="flex-[1.8] text-sm text-gray-400 text-center" style={{ fontFamily: fonts.bold }}>{t("attendanceHeader", "Attendance")}</Text>
                    <Text className="flex-[2] text-sm text-gray-400 text-center" style={{ fontFamily: fonts.bold }}>{t("actionsHeader", "Actions")}</Text>
                </View>
                ...
                {subjects.map((item, index) => {
                    ...
                    return (
                        <View className="flex-row items-center py-3 border-b border-gray-50" key={index}>
                            <Text className="flex-[2.2] text-base text-[#282828]" numberOfLines={2} style={{ fontFamily: fonts.regular }}>
                                {item.subject}
                            </Text>

                            <View className="flex-[1.8] gap-0.5 flex-row items-center justify-center">
                                <Text className={`text-base font-medium ${isEligible ? "text-[#43C17A]" : "text-red-500"}`} style={{ fontFamily: fonts.medium }}>
                                    {item.attendance}%
                                </Text>
                                <Text className="text-base text-gray-400" style={{ fontFamily: fonts.regular }}>/100%</Text>
                            </View>
                            ...
                            <View className="flex-[2] items-center">
                                <TouchableOpacity
                                    ...
                                    activeOpacity={0.7}
                                >
                                    <Text className="text-white text-base text-center" style={{ fontFamily: fonts.bold }}>
