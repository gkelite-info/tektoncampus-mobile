...
function ProgressRing({ value }: { value: number }) {
    ...
    const ringColor =
        safeValue >= 75 ? "#4ABF08" : safeValue >= 50 ? "#F59E0B" : safeValue > 0 ? "#FF3B30" : "#D9DDE3";
    ...
    return (
        <View style={{ width: size, height: size, justifyContent: "center", alignItems: "center", position: "relative" }}>
            ...
            <View style={{ 
                ...
                <Text style={{ fontSize: 10, color: ringColor === "#D9DDE3" ? "#9CA3AF" : ringColor, fontFamily: fonts.bold }}>
                    {safeValue}%
                </Text>
            </View>
        </View>
        ...
        <View className="w-full bg-transparent">
            <View className="mb-3 flex-col gap-3">
                <View className="flex-row items-center justify-between">
                    <Text className="text-[14.5px] tracking-tight text-gray-800" style={{ fontFamily: fonts.bold }}>
                        {t("Class Progress Overview")}
                    </Text>
                    <TouchableOpacity
                        activeOpacity={0.7}
                        className="flex-row items-center gap-1.5 rounded-md bg-[#43C17A] px-3 py-1.5 shadow-sm"
                    >
            ...
            <View className="flex-col gap-3">
                {rows.length === 0 ? (
                    <View className="bg-white rounded-xl p-8 text-center border border-gray-100 shadow-sm">
                        ...
                        >
                            <View className="flex-col gap-2.5">
                                <View className="flex-row items-start">
                                    <Text className="w-[125px] text-[#282828] text-[13px]" style={{ fontFamily: fonts.semiBold }}>
                                        {t("Subject")} :
                                    </Text>
                                    <Text
                                        className="flex-1 text-[#525252] text-[13px] pr-14"
                                        numberOfLines={1}
                                ...
                                <View className="flex-row items-center">
                                    ...
                                    <Text className="flex-1 text-[#525252] text-[13px]" style={{ fontFamily: fonts.regular }}>
                                        {row.attendance}
                                    </Text>
                                </View>
