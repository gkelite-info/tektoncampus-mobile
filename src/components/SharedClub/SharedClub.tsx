...
export default function SharedClub({ role }: SharedClubProps) {const { t } = useTranslation();
    ...
    if (!clubData) {
      return (
        <View style={tw`flex-1 items-center justify-center bg-[#F4F5F6] p-4`}>
                    <Text style={tw`text-xl font-bold text-[#16284F] mb-2`}>{t("Auto.Common.NoClubAssigned", "No Club Assigned")}</Text>
                    <Text style={tw`text-gray-500 font-medium text-center`}>{t("Auto.Common.Youarenotcurren", "You are not currently assigned as a Responsible Faculty or Mentor for any active clubs.")}
  ...
  if (clubStatus === "error") {
    return (
      <View style={tw`flex-1 items-center justify-center bg-[#F4F5F6] p-4`}>
                <Text style={tw`text-xl font-bold text-red-500 mb-2`}>{t("Auto.Common.ConnectionError", "Connection Error")}</Text>
                <Text style={tw`text-gray-500 font-medium text-center`}>{t("Auto.Common.Pleaserefreshto", "Please refresh to try loading your club again.")}
    ...
    <View style={tw`flex-1 bg-[#F4F5F6]`}>
        ...
        <View style={tw`flex-1`}>
          ...
          <View style={tw`flex-1 items-center justify-center p-4`}>
                                <Text style={tw`text-xl font-bold text-[#16284F] mb-2`}>{t("Auto.Common.NoClubJoined", "No Club Joined")}</Text>
                                <Text style={tw`text-gray-500 font-medium text-center`}>{t("Auto.Common.Youarenotcurren", "You are not currently a member of any club. Explore the available clubs and send a request to join one!")}

            </Text>
                            </View> :
          clubStatus === "pending" ?
          <View style={tw`flex-1 items-center justify-center p-4`}>
                                <Text style={tw`text-xl font-bold text-[#16284F] mb-2`}>{t("Auto.Common.RequestPending", "Request Pending")}</Text>
                                <Text style={tw`text-gray-500 font-medium text-center`}>{t("Auto.Common.Yourrequesttojo", "Your request to join")}
              {clubData?.name}{t("Auto.Common.iscurrentlypend", "is currently pending approval by the responsible faculty or mentors.")}
            </Text>
                            </View> :
