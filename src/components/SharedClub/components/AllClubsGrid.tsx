...
export default function AllClubsGrid({
  ...
  if (isLoading) {
    return <View style={tw`flex-1 items-center justify-center pt-20`}>
  ...
  return <FlatList data={clubs} keyExtractor={item => item.id.toString()} numColumns={1} contentContainerStyle={tw`pb-20 px-4`} renderItem={({
    ...
    return <View style={tw`bg-[#FB8000]/10 rounded-2xl flex-col items-center px-4 pb-5 pt-[60px] relative mt-16 mb-4`}>
      ...
      }} disabled={isButtonDisabled} style={tw`w-full py-3 rounded-xl shadow-md items-center justify-center ${buttonClass}`}>
