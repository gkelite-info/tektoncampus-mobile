...
import { View, TextInput, TouchableOpacity, ScrollView, ActivityIndicator, KeyboardAvoidingView, Platform, Alert, Keyboard } from 'react-native';
...
import {
  ...
  fetchSingleAnnouncement } from
"@/lib/helpers/clubActivity/announcementAPI";
...
export default function Announcements({ userRole, clubId, collegeId, roleType }: AnnouncementsProps) {const { t } = useTranslation();
    ...
    <KeyboardAvoidingView
      ...
      keyboardVerticalOffset={Platform.OS === "ios" ? 100 : 0}>
        ...
        <View style={tw`flex-1 items-center justify-center pt-20`}>
                        ...
                        <Text style={tw`text-lg font-bold text-[#16284F] mb-1`}>{t("Auto.Common.NoAnnouncements", "No Announcements Yet")}</Text>
                        <Text style={tw`text-sm text-gray-500 text-center px-6`}>{t("Auto.Common.Bethefirsttosha", "Be the first to share an update or important information with the club members!")}
          ...
          <View style={tw`items-center py-2`}>
            ...
            return (
              <View key={announcement.announcementId} style={tw`flex-row mb-4 items-start`}>
                  ...
                  <Text style={tw`text-[10px] font-bold text-gray-400 text-right mt-1`}>{t("Auto.Common.Edited", "Edited")}</Text>
                  }
                                    </View>
      ...
      <View style={[tw`p-4 bg-[#F4F5F6]`, { paddingBottom: isKeyboardVisible ? Platform.OS === 'android' ? keyboardHeight : 16 : Math.max(insets.bottom, 16) }]}>
          ...
          <TouchableOpacity onPress={() => {setEditingId(null);setInputValue("");}} style={tw`mr-3`}>
                                <Text style={tw`text-xs text-red-500 font-bold`}>{t("Auto.Common.Cancel", "Cancel")}</Text>
                            </TouchableOpacity>
          }
                        <TouchableOpacity
