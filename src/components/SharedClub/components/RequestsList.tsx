...
export default function RequestsList({ clubId, currentFilter, onChangeFilter }: { clubId: any, currentFilter: string, onChangeFilter: (f: string) => void }) {
    ...
    const { userId: facultyId } = useUser();
    ...
    const handleExecuteAction = async (type: "accept" | "reject" | "remove", reqItem: any) => {
        if (!facultyId) return Toast.show({ type: "error", text1: "User not authenticated" });
        ...
        try {
            ...
            Toast.show({ type: "success", text1: `Successfully ${type}ed!` });
            fetchRequests();
        } catch (error: any) {
        ...
        <View style={tw`flex-1 px-4`}>
            ...
            <View style={tw`flex-row gap-2 mb-4`}>
                {["all", "pending", "accepted"].map((filter) => (
                    <TouchableOpacity
                        ...
                        style={tw`rounded-full px-5 py-2 ${
                            currentFilter === filter ? "bg-[#16284F]" : "bg-[#E7E7E7]"
                        }`}
                    >
                        <Text style={tw`text-sm font-semibold capitalize ${
                            currentFilter === filter ? "text-white" : "text-[#000000]"
                        }`}>
                            {filter}
                        </Text>
                    </TouchableOpacity>
            ...
            <View style={tw`flex-row items-center rounded-full bg-[#EAEAEA] px-4 py-2.5 mb-4`}>
                <TextInput
                    ...
                    style={tw`flex-1 text-sm text-[#282828] p-0`}
            ...
            {isLoading ? (
                <ActivityIndicator size="large" color="#43C17A" style={tw`mt-10`} />
            ) : requests.length > 0 ? (
                <FlatList
                    data={requests}
                    ...
                    renderItem={({ item: req }) => (
                        <View style={tw`flex-row items-center justify-between rounded-lg bg-white p-3 shadow-sm border border-gray-100 mb-3`}>
                                ...
                                {req.status === "pending" ? (
                                    <>
                                        <TouchableOpacity
                                            ...
                                            onPress={() => handleExecuteAction("reject", req)}
                                            style={tw`rounded-md bg-[#FF2A2A] px-4 py-1.5 ${isActionLoading ? 'opacity-50' : ''}`}
                                        >
                                            <Text style={tw`text-xs font-semibold text-white`}>Reject</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity
                                            disabled={isActionLoading}
                                            onPress={() => handleExecuteAction("accept", req)}
                                            style={tw`rounded-md bg-[#43C17A] px-4 py-1.5 ${isActionLoading ? 'opacity-50' : ''}`}
                                        >
                                            <Text style={tw`text-xs font-semibold text-white`}>Accept</Text>
                                        </TouchableOpacity>
                                    </>
                                ) : req.status === "accepted" ? (
                                    <TouchableOpacity
                                        disabled={isActionLoading}
                                        onPress={() => handleExecuteAction("remove", req)}
                                        style={tw`rounded-md bg-[#16284F] px-4 py-1.5 ${isActionLoading ? 'opacity-50' : ''}`}
                                    >
                                        <Text style={tw`text-xs font-semibold text-white`}>Remove</Text>
                                    </TouchableOpacity>
                                ) : null}
                            </View>
                        </View>
