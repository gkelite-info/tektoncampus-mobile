...
export default function DeleteFolderModal({
  ...
  onConfirm,
  loading = false
}: DeleteFolderModalProps) {const { t } = useTranslation();
  if (!open) return null;
    ...
    <Modal
      ...
      <View className="flex-1 justify-center items-center bg-black/40 px-4">
        <View className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
          <Text className="mb-2 text-base font-semibold text-gray-900">{t("Auto.Common.Deletefolder", "Delete folder")}

          </Text>
          
          <Text className="text-sm text-gray-500 mb-6 leading-5">{t("Auto.Common.Areyousureyouwa", "Are you sure you want to delete")}
            {folderName || "this folder"}{t("Auto.Common.Thisactioncanno", "? This action cannot be undone.")}
          </Text>
