...
import { View, Text, TouchableOpacity, Image, TextInput, ActivityIndicator, Linking } from 'react-native';
...
const RoleFlair = ({ role }: { role?: string }) => {
    ...
    return (
        <View className={`px-2 py-0.5 rounded-full border ${isStaff ? "bg-blue-50 border-blue-100" : "bg-gray-100 border-gray-200"}`}>
...
const UserAvatar = ({ userId, name, photoUrl, size = 40 }: { userId: number, name?: string, photoUrl?: string | null, size?: number }) => {
    if (photoUrl) {
        return <Image source={{ uri: photoUrl }} style={{ width: size, height: size, borderRadius: size / 2 }} className="border border-gray-100" />;
    }
    return (
        <View style={[{ width: size, height: size, borderRadius: size / 2 }, getAvatarStyle(userId)]} className="items-center justify-center shadow-inner">
            <Text className="text-white font-bold" style={{ fontSize: size * 0.4 }}>{getInitials(name)}</Text>
        </View>
    );
};

export default function PostCard({ post, userId, fullName, currentUserPhoto, isHighlighted, debouncedSearch, onEditPost, onDeletePost }: any) {
    ...
    const handleEditCommentSubmit = async (commentId: number) => {
        ...
        if (res.success) {
            setComments(prev => prev.map(c => c.campusBuzzPostCommentId === commentId ? { ...c, comment: editingCommentText } : c));
    ...
    const handleShareAction = async (platform: string) => {
        ...
        const res = await shareCampusBuzzPost({ campusBuzzPostId: pId, sharedTo: platform }, userId);
    ...
    return (
        <View className={`bg-white rounded-xl border mb-4 shadow-sm ${isHighlighted && !debouncedSearch ? "border-[#43C17A] bg-[#fafffb]" : "border-gray-100"}`}>
            <View className="p-4">
                ...
                <View className="flex-row justify-between items-start mb-3 relative z-10">
                    ...
                    {post.createdBy === userId && (
                        <View className="relative">
                            ...
                            {menuOpen && (
                                <View className="absolute right-0 top-8 bg-white border border-gray-100 rounded-lg shadow-lg w-32 py-1 z-[999] elevation-5">
                                    <TouchableOpacity 
                                        onPress={() => { setMenuOpen(false); onEditPost(post); }}
                                    ...
                                    <TouchableOpacity 
                                        onPress={() => { setMenuOpen(false); onDeletePost(pId); }}
                ...
                {tagArray.length > 0 && (
                    <View className="flex-row flex-wrap gap-2 mt-3">
                        {tagArray.map((tag: string, i: number) => (
                            <View key={i} className="bg-[#EAF7F1] px-2.5 py-1 rounded-md">
                ...
                {post.imageUrl && (
                    <View className="mt-3 w-full h-[220px] bg-gray-50 rounded-xl overflow-hidden border border-gray-100">
                ...
                <View className="flex-row justify-between items-center mt-4 pt-3 border-t border-gray-50">
                    ...
                    <View className="relative">
                        ...
                        {sharedActive && (
                            <View className="absolute right-0 bottom-8 bg-white border border-gray-100 rounded-lg shadow-lg w-[180px] py-1 z-[999] elevation-5">
                                <TouchableOpacity 
                ...
                {commentsActive && (
                    <View className="mt-4 pt-3 border-t border-gray-50 flex-col gap-3">
                        {replyingTo && (
                            <View className="flex-row justify-between items-center bg-gray-100 px-3 py-1.5 rounded-lg mb-1">
                        ...
                        <View className="flex-row items-center gap-2">
                            ...
                            <TextInput 
                                placeholder={replyingTo ? t("Write a reply...", "Write a reply...") : t("Write a comment...", "Write a comment...")}
                                placeholderTextColor="#9CA3AF"
                                value={newCommentText}
                                onChangeText={setNewCommentText}
                                className="flex-1 h-10 px-4 bg-gray-100 rounded-full text-sm text-[#282828]"
                            />
                            <TouchableOpacity 
                                onPress={handleSubmitComment}
                                disabled={isSubmittingComment || !newCommentText.trim()}
                                className={`w-10 h-10 rounded-full items-center justify-center ${newCommentText.trim() ? "bg-[#43C17A]" : "bg-gray-300"}`}
                            >
                        ...
                        {isLoadingComments ? (
                            <ActivityIndicator size="small" color="#43C17A" className="my-4" />
                        ) : parentComments.length === 0 ? (
                            <Text className="text-center text-xs text-gray-400 py-2">{t("No comments yet", "No comments yet")}</Text>
                        ) : (
                            <View className="mt-2 flex-col gap-3">
                                {parentComments.map((comment: any) => {
                                    ...
                                    const renderComment = (c: any, isReply = false) => (
                                        <View key={c.campusBuzzPostCommentId} className={`flex-row gap-2 ${isReply ? "mt-3" : ""}`}>
                                            ...
                                            <View className="flex-1">
                                                        ...
                                                        {editingCommentId === c.campusBuzzPostCommentId ? (
                                                            <View className="mt-1">
                                                                <TextInput 
                                                                    ...
                                                                    className="bg-white px-2 py-1 text-xs border border-gray-200 rounded"
                                                                    autoFocus
                                                                />
                                                            ...
                                                            <Text className={`text-[#1c1e21] ${isReply ? "text-[12px]" : "text-[13px]"}`}>{c.comment}</Text>
                                                        )}
                                                    </View>

                                                    {c.commentedBy === userId && editingCommentId !== c.campusBuzzPostCommentId && (
                                                        <TouchableOpacity 
                                                            onPress={() => activeCommentMenuId === c.campusBuzzPostCommentId ? setActiveCommentMenuId(null) : setActiveCommentMenuId(c.campusBuzzPostCommentId)}
                                                            className="p-1"
                                                        >
                                                ...
                                                {activeCommentMenuId === c.campusBuzzPostCommentId && (
                                                    <View className="flex-row gap-3 mt-1 ml-1 bg-white px-2 py-1 rounded shadow-sm border border-gray-100 self-start">
                                                        ...
                                                        <TouchableOpacity onPress={() => { handleDeleteComment(c.campusBuzzPostCommentId); setActiveCommentMenuId(null); }}>
                                                ...
                                                {editingCommentId !== c.campusBuzzPostCommentId && (
                                                    <View className="flex-row items-center gap-3 px-1 mt-1">
                                                        ...
                                                        {!isReply && (
                                                            <TouchableOpacity onPress={() => setReplyingTo({ commentId: c.campusBuzzPostCommentId, userName: c.users?.fullName || "Member" })}>
                                    ...
                                    return (
                                        <View key={comment.campusBuzzPostCommentId} className="flex-col gap-1 mb-2">
                                            {renderComment(comment, false)}
                                            {replies.length > 0 && (
                                                <View className="pl-4 ml-4 border-l-2 border-gray-100">
