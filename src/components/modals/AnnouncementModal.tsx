...
import { Modal, View, TouchableOpacity, Platform, FlatList, TextInput, ScrollView, ActivityIndicator, Alert, Keyboard } from 'react-native';
  ...
  const renderHeader = () => {
    ...
    return <View className="bg-white">
            ...
            <View className="mt-4 border-b border-gray-100">
                <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-4 pb-3" contentContainerStyle={{
          gap: 8
        }}>
                    {TABS.map(tab => {
      ...
      paddingTop: Platform.OS === 'android' ? 40 : 0
        ...
        overflow: 'hidden'
              ...
              setIsAddPostOpen(true);
                ...
                {isLoading ? <View className="flex-1 px-4 pt-4">
                        {[1, 2, 3].map(i => <View key={i} className="mb-4 bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
          ...
          item
                                ...
                                <PostCard post={item} userId={userId} fullName={fullName} currentUserPhoto={profilePhoto} debouncedSearch={debouncedSearch} isHighlighted={highlightedPostId === item.campusBuzzPostId} onEditPost={(p: any) => {
            setEditPostData(p);
            setIsAddPostOpen(true);
          }} onDeletePost={confirmDeletePost} />
                            </View>} onEndReached={handleLoadMore} onEndReachedThreshold={0.5} contentContainerStyle={{
          paddingBottom: 40
        }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" onScrollBeginDrag={Keyboard.dismiss} />}

                <AddPostModal visible={isAddPostOpen} onClose={() => setIsAddPostOpen(false)} editData={editPostData} onSuccess={() => loadPosts(0, true)} />
            </SafeAreaView>
            </View>
        </Modal>;
}