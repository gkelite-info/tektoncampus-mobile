            ...
            <SafeAreaView className="flex-1 bg-white">
                ...
                >
                    ...
                    <ScrollView className="flex-1 px-4 py-3" keyboardShouldPersistTaps="handled">
                        <View className="mb-3">
                            <Text className="text-[13px] font-semibold text-[#282828] mb-1">Send To</Text>
                            <View className="flex-row gap-2">
                                {!initialEmail && (
                                    <View className="flex-1 border border-gray-300 rounded-md bg-white justify-center h-[50px] overflow-hidden">
                                        ...
                                        >
                                            <Picker.Item label="Select Audience" value="" color="#9CA3AF" style={{ fontSize: 13 }} />
                                            {rolesList.map((role) => (
                        ...
                        {!initialEmail && (
                            <View className="mb-3">
                                <Text className="text-[13px] font-semibold text-[#282828] mb-1">Filters</Text>
                                <View className="flex-row flex-wrap justify-between gap-y-2">
                                    <FilterPill label="Edu Type" value={edu} onChange={setEdu} options={eduList} valKey="collegeEducationId" nameKey="collegeEducationType" defaultLabel="All" />
                                    <FilterPill label="Branch" value={branch} onChange={setBranch} options={branchList} valKey="collegeBranchId" nameKey="collegeBranchType" defaultLabel="All" disabled={!edu} />
                                    <FilterPill label="Year" value={year} onChange={setYear} options={yearList} valKey="collegeAcademicYearId" nameKey="collegeAcademicYear" defaultLabel="All Years" disabled={!branch} />
                                    <FilterPill label="Sem" value={sem} onChange={setSem} options={semList} valKey="collegeSemesterId" nameKey="collegeSemester" defaultLabel="All" disabled={!year} />
                                    <FilterPill label="Sec" value={sec} onChange={setSec} options={secList} valKey="collegeSectionsId" nameKey="collegeSections" defaultLabel="All Sections" disabled={!year} />
                                </View>
                            </View>
                        )}

                        <View className="mb-3">
                            <Text className="text-[13px] font-semibold text-[#282828] mb-1">Cc</Text>
                            <TextInput
                        ...
                        <View className="mb-3">
                            <Text className="text-[13px] font-semibold text-[#282828] mb-1">Subject</Text>
                            <TextInput
                                className="w-full px-3 py-2 border border-gray-300 rounded-md text-[13px]"
                                placeholder="Subject"
                                value={subject}
