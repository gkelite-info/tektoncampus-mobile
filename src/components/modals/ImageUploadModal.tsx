...
export default function ImageUploadModal({ visible, onClose, userId, onUploadSuccess }: ImageUploadModalProps) {const { t } = useTranslation();
    ...
    <Modal visible={visible} transparent animationType="fade">
            <View className="flex-1 bg-black/50 justify-center items-center px-4">
                <View className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-xl relative">
                    ...
                    <Text className="text-xl font-bold text-[#1B2B5B] mb-2 text-center">{t("Auto.Common.UpdatePhoto", "Update Photo")}</Text>
                    <Text className="text-sm text-gray-500 mb-6 text-center">{t("Auto.Common.Chooseanoptiont", "Choose an option to set your profile picture")}</Text>

                    {isUploading ?
          <View className="py-8 items-center">
                            <ActivityIndicator size="large" color="#43C17A" />
                            <Text className="mt-4 font-medium text-gray-600">{t("Auto.Common.Uploading", "Uploading...")}</Text>
                        </View> :
          ...
          <View className="gap-3">
              ...
              className="w-full flex-row items-center p-4 rounded-xl border border-gray-200 bg-gray-50">
                                ...
                                <View>
                                    <Text className="text-[#282828] font-bold text-base">{t("Auto.Common.TakeaPhoto", "Take a Photo")}</Text>
                                    <Text className="text-gray-500 text-xs">{t("Auto.Common.Useyourcamera", "Use your camera")}</Text>
                                </View>
                            </TouchableOpacity>
              ...
              className="w-full flex-row items-center p-4 rounded-xl border border-gray-200 bg-gray-50">
                                ...
                                <View>
                                    <Text className="text-[#282828] font-bold text-base">{t("Auto.Common.ChoosefromGalle", "Choose from Gallery")}</Text>
                                    <Text className="text-gray-500 text-xs">{t("Auto.Common.Browseyourphoto", "Browse your photos")}</Text>
                                </View>
                            </TouchableOpacity>
                        </View>
          }
