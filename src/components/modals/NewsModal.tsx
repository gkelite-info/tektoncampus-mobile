...
import { Modal, View, Text, TouchableOpacity, ScrollView, ActivityIndicator, Linking, Platform, Alert } from 'react-native';
...
export default function NewsModal({ visible, onClose }: Props) {
    ...
    const handleDeleteEpaper = (ePaperId: number) => {
        Alert.alert(
            t("Delete EPaper", "Delete EPaper"),
            t("Are you sure you want to delete this e-paper?", "Are you sure you want to delete this e-paper?"),
            [
            ...
            <SafeAreaView className="flex-1 bg-white" edges={['top']}>
                {selectedArticleUrl ? (
                    <View className="flex-1 bg-white">
                        ...
                        <WebView source={{ uri: selectedArticleUrl }} className="flex-1" />
                    </View>
                ) : (
                    <View className="flex-1">
                        ...
                        <View className="flex-row border-b border-gray-100 bg-white">
                            <TouchableOpacity 
                                onPress={() => setActiveTab("epaper")}
                                className={`flex-1 py-3 border-b-2 ${activeTab === 'epaper' ? 'border-[#43C17A]' : 'border-transparent'}`}
                            >
                            ...
                            <TouchableOpacity 
                                ...
                                className={`flex-1 py-3 border-b-2 ${activeTab === 'news' ? 'border-[#43C17A]' : 'border-transparent'}`}
                            >
                        ...
                        <View className="flex-1 bg-gray-50/50">
                            {activeTab === 'epaper' && (
                                <ScrollView contentContainerStyle={{ padding: 16 }}>
                                    {isAdmin && (
                                        <TouchableOpacity className="w-full flex-row items-center justify-center gap-2 py-3 bg-white border border-[#43C17A] rounded-lg mb-4">
                                    ...
                                    {loadingEpapers ? (
                                        <View>
                                            {[1, 2, 3].map(i => (
                                                <View key={i} className="mb-4">
                                                    ...
                                                    {[1, 2].map(j => (
                                                        <View key={j} className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm flex-row items-center justify-between mb-3">
                                        ...
                                        <Text className="text-center text-gray-500 text-sm mt-8">{t("No EPapers available", "No EPapers available")}</Text>
                                    ) : (
                                        Object.entries(groupedEpapers).map(([date, papers]) => (
                                            <View key={date} className="mb-4">
                                                ...
                                                <View className="space-y-3">
                                                    {papers.map((paper) => (
                                                        <View key={paper.ePaperId} className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm flex-row items-center justify-between mb-3">
                                                            ...
                                                            <View className="flex-row items-center gap-2 ml-2">
                                                                ...
                                                                {isAdmin && (
                                                                    <TouchableOpacity 
                                                                        onPress={() => handleDeleteEpaper(paper.ePaperId)}
                                ...
                                <ScrollView contentContainerStyle={{ padding: 16 }}>
                                    {loadingNews ? (
                                        <View className="space-y-4">
                                            ...
                                            {[1, 2, 3].map(i => (
                                                <View key={i} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm mb-4">
                                        ...
                                        <View className="space-y-4">
                                            ...
                                            {news.map((item, index) => (
                                                <View key={index} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm mb-4">
