...
import { View, TouchableOpacity, Modal, ScrollView, SafeAreaView, TextInput } from 'react-native';
...
export default function SupportModal({ visible, onClose }: Props) {const { t } = useTranslation();
  const { role } = useUser();
            ...
            <SafeAreaView className="flex-1 bg-[#F3F4F6]">
                ...
                <ScrollView className="flex-1 p-4" contentContainerStyle={{ paddingBottom: 40 }}>
                    ...
                    <View className="flex-row justify-between mb-6">
                        <View className="flex-1 bg-[#E5E0FF] rounded-xl p-3 mr-2 items-center">
                            ...
                            <Text className="text-xs text-[#525252]">{t("Auto.Common.Previous", "Previous")}</Text>
                        </View>
                        <View className="flex-1 bg-[#F5E6CA] rounded-xl p-3 mx-1 items-center">
                            ...
                            <Text className="text-xs text-[#525252]">{t("Auto.Common.Pending", "Pending")}</Text>
                        </View>
                        <View className="flex-1 bg-[#D2EBE0] rounded-xl p-3 ml-2 items-center">
                            ...
                            <Text className="text-xs text-[#525252]">{t("Auto.Common.Resolved", "Resolved")}</Text>
                        </View>
                    </View>
                    ...
                    <View className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                        {}
                        <View className="mb-4">
                            <Text className="text-xs font-bold text-[#515151] mb-1">{t("Auto.Common.Role", "Role")}</Text>
                        ...
                        {}
                        <View className="mb-4">
                            <Text className="text-xs font-bold text-[#515151] mb-1">{t("Auto.Common.IssueRelated", "Issue Related")}</Text>
                            <TouchableOpacity
              ...
              <View className="mt-1 border border-gray-200 rounded-md bg-white">
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    setShowIssueDropdown(false);
                        ...
                        {}
                        <View className="mb-4">
                            <Text className="text-xs font-bold text-[#515151] mb-1">{t("Auto.Common.Description", "Description")}</Text>
                            <TextInput
                value={description}
                onChangeText={setDescription}
                placeholder={t("Auto.Attr.Providedetailed", "Provide detailed information about the issue you're facing...")}
                multiline
                numberOfLines={5}
                ...
                className="bg-white border border-[#CCCCCC] rounded-md p-3 text-[#525252] min-h-[100px]" />
                        ...
                        {}
                        <View className="mb-5">
                            <Text className="text-xs font-bold text-[#515151] mb-2">{t("Auto.Common.PriorityLevel", "Priority Level")}</Text>
                  ...
                  priority === "Low" ? "border-[#43C17A] bg-[#F4FAF6]" : "border-[#CCCCCC] bg-white"}`
                                    ...
                                    <Text className={`text-xs font-bold ${priority === "Low" ? "text-[#282828]" : "text-[#515151]"}`}>{t("Auto.Common.Low", "Low")}</Text>
                                </TouchableOpacity>
                  ...
                  priority === "Medium" ? "border-[#43C17A] bg-[#F4FAF6]" : "border-[#CCCCCC] bg-white"}`
                                    ...
                                    <Text className={`text-xs font-bold ${priority === "Medium" ? "text-[#282828]" : "text-[#515151]"}`}>{t("Auto.Common.Medium", "Medium")}</Text>
                                </TouchableOpacity>
                  ...
                  priority === "High" ? "border-[#43C17A] bg-[#F4FAF6]" : "border-[#CCCCCC] bg-white"}`
                                    ...
                                    <Text className={`text-xs font-bold ${priority === "High" ? "text-[#282828]" : "text-[#515151]"}`}>{t("Auto.Common.High", "High")}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                        ...
                        {}
                        <View className="mb-4">
                            <Text className="text-xs font-bold text-[#515151] mb-1">{t("Auto.Common.UploadProofImag", "Upload Proof (Images/Video)")}</Text>
                            <TouchableOpacity className="border-2 border-dashed border-[#A8D5BA] bg-[#F8FCF9] rounded-xl p-8 items-center justify-center">
                                ...
                                <Text className="text-sm text-[#282828] mb-1">{t("Auto.Common.Tapto", "Tap to")}
                  <Text className="text-[#27AE60] font-bold">{t("Auto.Common.browse", "browse")}</Text>
                                </Text>
                                <Text className="text-xs text-[#888888]">{t("Auto.Common.SupportJPGPNGPD", "Support: JPG, PNG, PDF")}</Text>
                            </TouchableOpacity>
                        </View>

                        {}
                        <TouchableOpacity className="bg-[#43C17A] py-3.5 rounded-xl items-center mt-2">
                            <Text className="text-white font-bold text-base">{t("Auto.Common.SubmitRequest", "Submit Request")}</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
            </SafeAreaView>
