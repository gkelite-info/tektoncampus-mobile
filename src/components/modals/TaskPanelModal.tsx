...
export default function TaskPanelModal({
    ...
    const handleDelete = async (taskId: number) => {
        ...
        try {
            const res =
                role === "student"
                    ? await deactivateStudentTask(taskId)
                    : await deactivateFacultyTask(taskId);
            ...
            console.error(error);
            Toast.show({ type: "error", text1: t("An error occurred while deleting") });
        } finally {
            setIsDeleting(false);
        }
    };
                        ...
                        <View className="flex-row items-center gap-3">
                            {(role === "faculty" || (role === "student" && activeView === "student")) && (
                                <TouchableOpacity
                                    ...
                                    className="flex-row items-center gap-2 px-3 py-1.5 rounded-full border border-[#43C17A]"
                                >
                        ...
                        <View className="bg-white rounded-2xl shadow-sm p-4 flex-1">
                            <View className="flex-row justify-between items-center mb-4">
                                <View className="flex-row items-center">
                                    ...
                                    {role === "faculty" && (
                                        <Text className="text-[#282828] font-bold">
                                            {t("My Tasks")}
                                        </Text>
                                    )}
                                </View>
                                ...
                                {(isLoadingTasks || loading) ? (
                                    ...
                                    tasksToShow.map((task) => (
                                        <View
                                            ...
                                            className="bg-[#E8F8EF] rounded-xl mb-3 p-3 flex-row justify-between"
                                        >
                                                ...
                                                <View className="flex-row gap-2 mt-2">
                                                    {(role === "faculty" || (role === "student" && activeView === "student")) && (
                                                        <>
                                                            ...
                                                            <TouchableOpacity
                                                                onPress={() => confirmDelete(task.facultyTaskId)}
                                                                className="p-1.5 rounded-full bg-red-100/50"
                                                            >
