...
export function Pagination({
  ...
  onPageChange,
  roundedBottom
}: PaginationProps) {const { t } = useTranslation();
    ...
    <View className={`p-4 bg-white border-t border-gray-200 mt-auto items-center ${roundedBottom || ""}`}>
            <View className="flex-col gap-y-3 w-full items-center">
                ...
                <View className="flex-row justify-center w-full">
                    <Text className="text-xs text-gray-700">{t("Auto.Common.Showing", "Showing")}
            <Text className="font-semibold">{startIndex + 1}</Text>{t("Auto.Common.to", "to")}{" "}
                        <Text className="font-semibold">{endIndex}</Text>{t("Auto.Common.of", "of")}{" "}
                        <Text className="font-semibold">{totalItems}</Text>{t("Auto.Common.results", "results")}
          </Text>
                </View>
